...
// Handle listing boundaries for operator or driver
if (isset($data['action']) && $data['action'] === 'list') {
    try {
        ...
        if (isset($data['operator_id'])) {
            ...
            error_log("Fetching boundaries for driver_id: " . $driver_id);
            $stmt = $conn->prepare("
                SELECT 
                    bp.*,
                ...
                LEFT JOIN jeepney_assignments ja ON bp.jeepney_id = ja.jeepney_id AND ja.status = 'Active'
                WHERE bp.driver_id = ? 
                ORDER BY bp.paid_at DESC
            ");
            ...
            while ($row = $result->fetch_assoc()) {
                $boundaries[] = [
                    'id' => $row['id'],
                    'operator' => $row['operator_firstName'] . ' ' . $row['operator_lastName'],
            ...
            echo json_encode(['success' => false, 'message' => 'Missing operator_id or driver_id.']);
            exit;
        }
