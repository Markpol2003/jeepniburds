...
<div class="col-md-6 mb-4">
    <div class="card shadow-sm">
                        ...
                        <div class="col-md-6">
                            <form id="boundaryForm">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" name="reference_number" id="boundaryReference" required>
                                </div>
                                <div class="mb-3">
        ...
        <?php if ($page === 'boarding'): ?>
            ...
            function fetchBoardingEvents() {
                ...
                .then(data => {
                    ...
                    if (data.success && data.events && data.events.length > 0) {
                        let html = `<div class='table-responsive'><table class='table table-hover table-borderless align-middle modern-boarding-table'>
                            <thead class='table-light'>
                                <tr>
                                    <th><i class='bi bi-person-circle text-primary'></i> Passenger</th>
                                    <th><i class='bi bi-clipboard-check text-info'></i> Event</th>
                                    <th><i class='bi bi-clock-history text-secondary'></i> Time</th>
                                    <th><i class='bi bi-hand-thumbs-up text-success'></i> Likes</th>
                                </tr>
                            </thead>
                            <tbody>`;
                        data.events.forEach(ev => {
                            // Simulate likes as a random number for demo (replace with real count if backend is ready)
                            const likes = Math.floor(Math.random() * 5) + 1;
                            html += `<tr>
                                <td><span class='fw-semibold'><i class='bi bi-person-circle me-1 text-primary'></i>${ev.passenger}</span></td>
                                <td><span class='badge bg-info'>${ev.event_type}</span></td>
                                <td><span class='text-muted'><i class='bi bi-clock'></i> ${new Date(ev.event_time).toLocaleString()}</span></td>
                                <td><span class='badge bg-success px-3 py-2' data-bs-toggle='tooltip' title='Number of passengers who clicked &quot;I\'m here&quot;'><i class='bi bi-hand-thumbs-up-fill me-1'></i>${likes}</span></td>
                            </tr>`;
                        });
                        html += '</tbody></table></div>';
            ...
            <style>
            .modern-boarding-table th, .modern-boarding-table td {
                vertical-align: middle;
                font-size: 1rem;
            }
            .modern-boarding-table thead th {
                background: linear-gradient(90deg, #e3f2fd 0%, #bbdefb 100%);
                color: #1976d2;
                font-weight: 700;
                border: none;
            }
            .modern-boarding-table tbody tr {
                transition: background 0.2s;
            }
            .modern-boarding-table tbody tr:hover {
                background: #f1f8e9;
            }
            .modern-boarding-table .badge {
                font-size: 1rem;
                border-radius: 8px;
                font-weight: 600;
            }
