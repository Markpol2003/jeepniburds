...
try {
    ...
    $stmt = $conn->prepare("INSERT INTO membership_payments (user_id, amount, method, receipt_number, status, reference_number) VALUES (?, ?, ?, ?, 'Pending', ?)");
    $amount = 1000; // Fixed amount
    $referenceNumber = '';
    
    // Set reference number based on payment method
    ...
    $stmt->bind_param("idsss", $userId, $amount, $paymentMethod, $receiptNumber, $referenceNumber);
