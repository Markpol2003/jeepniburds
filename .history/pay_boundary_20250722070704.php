...
if (isset($data['driver_id'], $data['amount'], $data['payment_method'])) {
    try {
        ...
        $assignmentStmt = $conn->prepare("SELECT operator_id, id as jeepney_id FROM jeepney_assignments WHERE driver_id = ? AND status = 'Active' ORDER BY assigned_date DESC LIMIT 1");
...
if (isset($data['action']) && $data['action'] === 'list') {
    ...
    try {
        ...
        error_log("[BOUNDARY_LIST] operator_id received: " . $operator_id);
        
        // Only fetch boundaries with status 'Pending' or 'Collected' (exclude deleted/cancelled if any)
        $stmt = $conn->prepare("
            SELECT 
                bp.*,
                u.firstName, 
        ...
        error_log("Returned boundaries for operator_id $operator_id: " . json_encode($boundaries));
