...
if (isset($data['action']) && $data['action'] === 'list') {
    ...
    try {
        ...
        // Check if there are any boundary payments at all
        $allPayments = $conn->query("SELECT * FROM boundary_payments");
        error_log("Total boundary payments in database: " . $allPayments->num_rows);
        
        // Check if there are any payments for this operator
        $operatorPayments = $conn->query("SELECT * FROM boundary_payments WHERE operator_id = " . $operator_id);
        error_log("Boundary payments for operator " . $operator_id . ": " . $operatorPayments->num_rows);
        $stmt = $conn->prepare("
