    ...
    <div class="col">
        <div class="card text-center h-100">
            <div class="card-body">
                ...
                <a href="cooperative_fund.php" class="btn btn-warning">Collect Funds</a>
            </div>
        </div>
    </div>
</div>
...
<?php if (isset($_GET['page']) && $_GET['page'] === 'cooperative_funds'): ?>
    ...
    <!-- Modal for viewing confirmed payment details -->
    <div class="modal fade" id="coopFundViewModal" tabindex="-1" aria-labelledby="coopFundViewModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header bg-info text-white">
            <h5 class="modal-title" id="coopFundViewModalLabel">Cooperative Fund Payment Details</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
            <ul class="list-group list-group-flush" id="coopFundModalDetails">
            ...
            if (data.success && data.funds.length > 0) {
                ...
                data.funds.forEach(f => {
                    ...
                    if (!isCollected) {
                        ...
                        html += `<button class='btn btn-sm btn-info' onclick='viewCoopFund(${JSON.stringify(f).replace(/'/g, "&#39;")})'><i class='bi bi-eye'></i> View</button>`;
                    }
                    html += `</td></tr>`;
                });
    ...
    function confirmCoopFund(id, btn) {
        ...
        .then(data => {
            if (data.success) {
                ...
                btn.replaceWith(`<button class='btn btn-sm btn-info' onclick='viewCoopFundFromRow(this)'><i class='bi bi-eye'></i> View</button>`);
            } else {
                btn.disabled = false;
    ...
    // Modal view logic
    function viewCoopFund(f) {
        // f is an object with payment details
        let details = '';
        details += `<li class='list-group-item'><strong>Member:</strong> ${f.member}</li>`;
        details += `<li class='list-group-item'><strong>Amount:</strong> ₱${f.amount}</li>`;
        details += `<li class='list-group-item'><strong>Method:</strong> ${f.payment_method}</li>`;
        details += `<li class='list-group-item'><strong>Date:</strong> ${f.paid_at}</li>`;
        details += `<li class='list-group-item'><strong>Status:</strong> <span class='badge bg-success'>${f.status}</span></li>`;
        document.getElementById('coopFundModalDetails').innerHTML = details;
        var modal = new bootstrap.Modal(document.getElementById('coopFundViewModal'));
        modal.show();
    }
    // For dynamic row button after confirm
    function viewCoopFundFromRow(btn) {
        // Find the row and extract details
        const row = btn.closest('tr');
        const cells = row.querySelectorAll('td');
        let details = '';
        details += `<li class='list-group-item'><strong>Member:</strong> ${cells[0].textContent}</li>`;
        details += `<li class='list-group-item'><strong>Amount:</strong> ${cells[1].textContent}</li>`;
        details += `<li class='list-group-item'><strong>Method:</strong> ${cells[2].textContent}</li>`;
        details += `<li class='list-group-item'><strong>Date:</strong> ${cells[3].textContent}</li>`;
        details += `<li class='list-group-item'><strong>Status:</strong> <span class='badge bg-success'>Confirmed</span></li>`;
        document.getElementById('coopFundModalDetails').innerHTML = details;
        var modal = new bootstrap.Modal(document.getElementById('coopFundViewModal'));
        modal.show();
    }
