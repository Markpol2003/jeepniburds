...
try {
    ...
    try {
        // Insert payment - Changed payment_method to method
        $stmt = $conn->prepare("INSERT INTO membership_payments (user_id, amount, method, receipt_number, status, created_at) VALUES (?, ?, ?, ?, 'Pending', NOW())");
        if (!$stmt) {
