...
<nav class="navbar navbar-expand-lg navbar-dark">
    ...
    <a class="nav-link position-relative" href="#" role="button" id="notifDropdown" data-bs-toggle="dropdown" aria-expanded="false">
    ...
    <ul class="dropdown-menu dropdown-menu-end shadow-sm p-2" style="min-width: 280px;">
    <li class="dropdown-header fw-bold text-primary d-flex justify-content-between align-items-center">
        <span>Orientation Requests</span>
        <?php if ($pendingRequests > 0): ?>
            <a href="#" class="text-danger small fw-normal" onclick="clearNotifications(event)">🗑️ Clear All</a>
        <?php endif; ?>
    </li>
    ...
    <?php if ($pendingRequests > 0): ?>
        <?php while ($notif = $notifResult->fetch_assoc()): ?>
            <li class="dropdown-item small">
                ...
                <strong><?= htmlspecialchars($notif['firstName'] . ' ' . $notif['lastName']); ?></strong><br>
                <small class="text-muted">🕒 <?= date("M d, Y h:i A", strtotime($notif['requested_at'])) ?></small>
            </li>
        <?php endwhile; ?>
        <li><hr class="dropdown-divider"></li>
        <li>
            <a class="dropdown-item text-center text-primary fw-semibold" href="verify_applications.php">
                📋 View All Applications
            </a>
        </li>
    <?php else: ?>
        <li class="dropdown-item text-muted small">No new orientation requests.</li>
    <?php endif; ?>
</ul>

    <ul class="dropdown-menu dropdown-menu-end shadow-sm p-2" style="min-width: 250px;">
        <li class="dropdown-header fw-bold text-primary">Orientation Notifications</li>
        <?php if ($pendingRequests > 0): ?>
            <li><a class="dropdown-item text-dark" href="verify_applications.php">
                🔔 <?= $pendingRequests ?> new request<?= $pendingRequests > 1 ? 's' : '' ?> pending
            </a></li>
        <?php else: ?>
            <li class="dropdown-item text-muted small">No new orientation requests.</li>
        <?php endif; ?>
    ...
    <a class="nav-link" href="#" data-bs-toggle="modal" data-bs-target="#profileModal">
        <i class="bi bi-person-circle"></i> Profile
                ...
                <li class="nav-item">
                    <a class="nav-link <?= (isset($_GET['page']) && $_GET['page'] === 'collect_coop_funds') ? 'active' : '' ?>" href="?page=collect_coop_funds">
                        <i class="bi bi-piggy-bank"></i> Collect Cooperative Funds
                    </a>
                </li>
...
<div class="row row-cols-1 row-cols-md-2 row-cols-xl-3 row-cols-xxl-4 g-4 mt-4">
    ...
    <div class="col">
        <div class="card text-center h-100">
            <div class="card-body">
                ...
                <a href="?page=collect_coop_funds" class="btn btn-warning">Collect Funds</a>
            </div>
        </div>
    </div>
</div>
...
<?php if (isset($_GET['page']) && $_GET['page'] === 'collect_coop_funds'): ?>
    ...
    function confirmCoopFund(id, btn) {
        ...
        .then(data => {
            if (data.success) {
                ...
                btn.closest('tr').querySelector('td:nth-child(5) span').textContent = 'Collected';
                btn.replaceWith('-');
            } else {
                btn.disabled = false;
