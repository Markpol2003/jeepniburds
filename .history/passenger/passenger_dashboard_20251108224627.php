    ...
    (function(){
       ...
       function drawMap(){
        ...
        function formatWaitTime(hereAtStr) {
            ...
            try {
                let hereAt;
                if (hereAtStr.includes('T')) {
                    hereAt = new Date(hereAtStr);
                } else {
                    // MySQL datetime format: "YYYY-MM-DD HH:MM:SS"
                    hereAt = new Date(hereAtStr.replace(' ', 'T'));
                }
                
                // Check if date is valid
                if (isNaN(hereAt.getTime())) {
                    console.error('Invalid wait time:', hereAtStr);
                    return '';
                }
                
                const now = Date.now();
                const waitMs = now - hereAt.getTime();
                if (waitMs < 0) return '';
