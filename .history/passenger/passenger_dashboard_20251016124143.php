...
<head>
    ...
    .content-container { background: transparent; position: relative; z-index: 1; }
    .content { max-width: 1160px; margin: 0 auto; position: relative; z-index: 1; }
...
<div class="text-center mt-3">
    <button type="button" class="btn apply-now-btn btn-lg shadow-lg px-5 py-3" data-bs-toggle="modal" data-bs-target="#applyModal">
        Apply Now
    </button>
...
<?php else: ?>
    ...
    <div class="text-center mt-3">
        <button type="button" class="btn apply-now-btn btn-lg shadow-lg px-5 py-3" data-bs-toggle="modal" data-bs-target="#applyModal" onclick="console.log('🎯 Button clicked!'); var modal = document.getElementById('applyModal'); modal.style.display='block'; modal.classList.add('show'); document.body.classList.add('modal-open'); var backdrop = document.createElement('div'); backdrop.className = 'modal-backdrop fade show'; backdrop.id = 'customBackdrop'; document.body.appendChild(backdrop); console.log('✅ Modal should be visible now!');">
            Apply Now
        </button>
        ...
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-6">
                        <div class="mb-3">
                            <label class="form-label">Origin Landmark</label>
                            <select id="resOrigin" class="form-select"></select>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Destination Landmark</label>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="d-grid gap-2">
                                <button id="btnReserve" class="btn btn-primary"><i class="bi bi-send"></i> Reserve</button>
                                <button id="btnHere" class="btn btn-outline-primary" disabled><i class="bi bi-geo-alt"></i> I'm Here</button>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="card position-sticky" style="top: 0; z-index: 2;">
                            <div class="card-header">My Active Reservations</div>
                            <div class="card-body p-2">
                                <div id="resList" class="small text-muted">No active reservations</div>
                            </div>
                        </div>
                        <div id="resMap" style="height: 280px; border-radius: 12px;" class="mt-3"></div>
                    </div>
                </div>
            </div>
        </div>
...
<div class="modal fade" id="applyModal" tabindex="-1" aria-labelledby="applyModalLabel" aria-hidden="true" style="z-index: 10000;">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header gradient-header text-white">
                <h5 class="modal-title">Apply as Driver/Operator</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" onclick="var modal = document.getElementById('applyModal'); modal.style.display='none'; modal.classList.remove('show'); document.body.classList.remove('modal-open'); var backdrop = document.getElementById('customBackdrop'); if(backdrop) backdrop.remove();"></button>
            </div>
            <div class="modal-body">
...
<script>
    ...
    // Removed URL-based auto-open; using modal trigger only

    // Fallback: wire up Apply Now button to open modal if data attributes fail
    console.log('🔍 Looking for Apply Now buttons...');
    const applyButtons = document.querySelectorAll('.apply-now-btn');
    console.log('✅ Found', applyButtons.length, 'Apply Now button(s)');
    
    if (applyButtons && applyButtons.length) {
        applyButtons.forEach((btn, index) => {
            console.log(`🔘 Setting up click handler for button ${index + 1}`);
            console.log('Button element:', btn);
            console.log('Button computed style pointer-events:', window.getComputedStyle(btn).pointerEvents);
            console.log('Button computed style z-index:', window.getComputedStyle(btn).zIndex);
            console.log('Button position:', window.getComputedStyle(btn).position);
            
            btn.addEventListener('click', function(e) {
                console.log('🎯 Apply Now button clicked!', e);
                ...
                try {
                    ...
                    console.log('Bootstrap Modal available:', !!(window.bootstrap && bootstrap.Modal));
                    ...
                    if (modalEl && window.bootstrap && bootstrap.Modal) {
                        ...
                        console.log('⚠️ Using manual fallback to open modal...');
                        // Manual fallback if Bootstrap JS is not available
                        e.preventDefault();
                        ...
                        console.log('✅ Modal display and class added');
                        // Add backdrop
    ...
    console.log('✅ Apply Now button setup complete!');
</script>
