...
<head>
    ...
    /* Fix: Allow clicks on modal and close button properly */
    body.modal-open {
        overflow: hidden !important;
        padding-right: 0 !important; /* Prevent body shift on scroll lock */
    }

    /* CRITICAL: Modal z-index fix - backdrop MUST be below modal */
    .modal-backdrop {
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">LTFRB Region XI Official Fare Matrix</h5>
            ...
            .then(r=>r.json()).then(d=>{
                ...
                box.innerHTML = d.reservations.map(r=>{
                    ...
                    if(r.here_at && !r.boarded_at){
                        ...
                        const m=Math.floor(sec/60), s=sec%60; waitTxt = `<div class="mt-2"><i class="bi bi-clock-history text-warning"></i> <strong>Wait:</strong> ${m}m ${s}s</div>`;
                    }
                    let etaTxt = '';
                    if(r.eta_time && !r.boarded_at){
                        const etaMs = new Date(r.eta_time).getTime() - Date.now();
                        const etaMin = Math.max(0, Math.ceil(etaMs/60000));
                        etaTxt = `<div class="mt-2"><i class="bi bi-truck text-info"></i> <strong>ETA:</strong> ${etaMin} min</div>`;
                    }
                    const useDisc = document.getElementById('resUseDisc').checked;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // CRITICAL FIX: Fare matrix modal backdrop and clickability
    const fareModal = document.getElementById('fareModal');
    if (fareModal) {
        fareModal.addEventListener('shown.bs.modal', function () {
            console.log('✅ Fare modal shown - fixing z-index');
            
            // Remove ALL duplicate backdrops
            const backdrops = document.querySelectorAll('.modal-backdrop');
            if (backdrops.length > 1) {
                console.warn('⚠️ Removing', backdrops.length - 1, 'duplicate backdrops');
                for (let i = 1; i < backdrops.length; i++) {
                    backdrops[i].remove();
                }
            }
            
            // Force correct z-index on backdrop
            const backdrop = document.querySelector('.modal-backdrop');
            if (backdrop) {
                backdrop.style.zIndex = '1040';
                backdrop.style.pointerEvents = 'auto'; // Allow clicking to close
            }
            
            // Force modal and content above backdrop
            fareModal.style.zIndex = '1050';
            fareModal.style.pointerEvents = 'auto';
            
            const dialog = fareModal.querySelector('.modal-dialog');
            if (dialog) {
                dialog.style.pointerEvents = 'auto';
                dialog.style.zIndex = '1';
            }
            
            const content = fareModal.querySelector('.modal-content');
            if (content) {
                content.style.pointerEvents = 'auto';
            }
            
            // Ensure close button is clickable
            const closeBtn = fareModal.querySelector('.btn-close');
            if (closeBtn) {
            ...
            if (backdrop) {
                backdrop.addEventListener('click', function() {
                    ...
                    const modalInstance = bootstrap.Modal.getInstance(fareModal);
                    if (modalInstance) {
                        modalInstance.hide();
                    }
                }, { once: true });
        ...
        fareModal.addEventListener('hidden.bs.modal', function () {
            console.log('✅ Fare modal hidden - cleaning up');
