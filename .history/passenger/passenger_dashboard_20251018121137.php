...
<head>
    ...
    /* CRITICAL FIX: Ensure modal is always clickable */
    .modal-backdrop {
    ...
    /* Modal container - pass through clicks except on dialog */
    .modal {
        position: fixed !important;
        top: 0 !important;
        left: 0 !important;
    ...
    /* Make ONLY the dialog clickable */
    .modal-dialog {
        pointer-events: auto !important;
...
<script>
// AGGRESSIVE modal interaction fix
document.addEventListener('DOMContentLoaded', function() {
    if (typeof bootstrap !== 'undefined') {
        ...
        if (applyModal) {
            // AGGRESSIVE FIX: Force modal to be clickable when shown
            applyModal.addEventListener('show.bs.modal', function (e) {
                console.log('🔓 Modal opening - forcing clickable state');
                
                // Remove any potential blocking styles
                document.querySelectorAll('.sidebar, .content-wrapper, .content-container, .content').forEach(el => {
                    el.style.zIndex = '1';
            ...
            applyModal.addEventListener('shown.bs.modal', function (e) {
                console.log('✅ Modal opened - verifying clickable state');
                
                const modalDialog = this.querySelector('.modal-dialog');
                const modalContent = this.querySelector('.modal-content');
                
                // Force pointer events on critical elements
                if (modalDialog) {
                ...
                // Force pointer events on all form controls
                this.querySelectorAll('input, select, button, textarea, label').forEach(el => {
                ...
                setTimeout(() => {
                    ...
                    if (firstInput) {
                        firstInput.focus();
                        console.log('✅ Focused first input:', firstInput.id || firstInput.name);
                    }
                }, 150);
                
                console.log('✅ All modal elements forced to clickable state');
            });
