...
<head>
    ...
    /* Modal fixes - ensure modals always on top of all dashboard layers */
    .modal {
...
@media (min-width: 1200px) {
  .status-cards-container { grid-template-columns: 1fr; }
    ...
    <div class="text-center mb-3" style="background: #fffbeb; padding: 10px; border-radius: 8px;">
        <button type="button" class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#applyModal" onclick="console.log('Test button clicked!')">
            🧪 TEST - Click to Open Apply Modal
        </button>
    </div>
...
<!-- Bootstrap Verification and Manual Modal Trigger -->
<script>
// Verify Bootstrap is loaded and modals work
document.addEventListener('DOMContentLoaded', function() {
    console.log('🔄 DOM Content Loaded - Starting modal setup...');
    ...
    if (typeof bootstrap !== 'undefined') {
        ...
        console.log('🔍 Looking for applyModal...', applyModal ? 'FOUND!' : 'NOT FOUND');
        
        if (applyModal) {
            // Find ALL buttons with data-bs-target="#applyModal"
            ...
            applyButtons.forEach((btn, index) => {
                console.log(`   Button ${index + 1}:`, btn.id || 'no id', btn.textContent.trim());
                
                // Add manual click handler as backup
                btn.addEventListener('click', function(e) {
                    e.preventDefault();
                    console.log('🖱️ Button clicked:', this.id);
                    
                    try {
                        const modalInstance = new bootstrap.Modal(applyModal, {
                            backdrop: true,
                            keyboard: true,
                            focus: true
                        });
            ...
            // Test if we can manually create a modal instance
            try {
                const testModal = new bootstrap.Modal(applyModal);
                console.log('✅ Modal instance can be created');
            } catch (error) {
                console.error('❌ Cannot create modal instance:', error);
            }
        } else {
            console.error('❌ Apply Modal NOT found in DOM');
        }
    } else {
