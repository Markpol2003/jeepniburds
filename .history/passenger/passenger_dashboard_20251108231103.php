    ...
    (function(){
       ...
       function drawMap(){
        ...
        function formatEtaTime(etaTimeStr, serverMinutes) {
            // Prefer server-calculated minutes if available (more accurate, avoids timezone issues)
            if (serverMinutes !== null && serverMinutes !== undefined) {
                const mins = parseInt(serverMinutes);
                if (mins > 1) {
                    // For display, we show minutes with seconds based on server minutes
                    // Calculate seconds from the datetime for more precision
                    if (etaTimeStr) {
                        try {
                            ...
                            // Fall through to simple minutes display
                        }
                    }
                    // Fallback: just show minutes
                    return `${mins} min`;
                } else if (mins === 1) {
                    return '1 min';
                } else {
                    // 0 or less - driver is arriving
        ...
        function updateLiveTimers() {
            ...
            items.forEach(item => {
                ...
                if (etaTimeAttr && !boardedAttr) {
                    ...
                    if (etaEl) {
                        // Always recalculate from datetime for live updates (server minutes become stale)
                        // This ensures accuracy as time passes
                        const formatted = formatEtaTime(etaTimeAttr, null);
                        if (formatted) {
                            etaEl.innerHTML = `<i class="bi bi-truck text-info"></i> <strong>ETA:</strong> ${formatted}`;
                        } else {
                            // If time has passed or calculation failed, show "Driver is arriving"
