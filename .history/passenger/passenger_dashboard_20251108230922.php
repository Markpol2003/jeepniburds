...
<?php endif; ?>
    ...
    <style>
        ...
        .action-btn-here:hover:not(:disabled) {
            transform: translateY(-2px);
    ...
    <div class="py-3">
        <div class="card shadow-lg reservation-card border-0">
            <div class="card-header text-white d-flex align-items-center" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 20px;">
                <i class="bi bi-bookmark-check-fill me-2 fs-4"></i>
                <h5 class="mb-0 fw-bold">Make a Reservation</h5>
            </div>
            <div class="card-body p-4">
                <div class="row g-4">
                        ...
                        <div class="row g-4">
                            <div class="col-12">
                                <div class="mb-4">
                                    ...
                                    <select id="resOrigin" class="form-select form-select-lg"></select>
                                </div>
                                <div class="mb-4">
                                    <label class="label-with-icon">
                                    ...
                                    <select id="resDest" class="form-select form-select-lg"></select>
                                </div>
                                    ...
                                    <div class="fare-card">
                                        <div class="d-flex justify-content-between align-items-center mb-3">
                                            <div>
                                                ...
                                                <div class="h4 mb-0"><i class="bi bi-signpost-2"></i> <span id="resKm">-</span> km</div>
                                            </div>
                                            <div class="text-end">
                                                <div class="fare-label">Amount to Pay</div>
                                                <div class="fare-amount">₱<span id="resPayable">-</span></div>
                                            </div>
                                        </div>
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" id="resUseDisc" style="width: 20px; height: 20px;">
                                            <label for="resUseDisc" class="form-check-label ms-2">
                                                <i class="bi bi-person-badge"></i> I am eligible for discounted fare
                                            </label>
                                        </div>
                                        <div class="mt-3 pt-3" style="border-top: 1px solid rgba(255,255,255,0.2);">
                                            <small class="d-block mb-1"><i class="bi bi-info-circle"></i> Please prepare exact fare (LTFRB compliance)</small>
                                            <small class="d-block" id="resFareMatch"></small>
                                        </div>
                                    </div>
                                </div>
                                
                                <!-- Compliance Note -->
                                <div class="compliance-note-compact mb-4">
                                    <div class="compliance-card">
                                        <div class="d-flex align-items-start">
                                            <i class="bi bi-exclamation-triangle-fill me-3 fs-4"></i>
                                            <div class="flex-grow-1">
                                                <div class="fw-bold mb-1">Important: Exact Fare Required</div>
                                                <small>Drivers may not have change. 
                                                <a href="#" class="text-white text-decoration-underline" data-bs-toggle="modal" data-bs-target="#fareModal">View fare matrix</a></small>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="d-grid gap-3">
                                    <button id="btnReserve" class="btn btn-primary action-btn action-btn-reserve text-white">
                                        <i class="bi bi-send-fill me-2"></i> Reserve Now
                                    </button>
                                    <button id="btnHere" class="btn btn-primary action-btn action-btn-here text-white" disabled>
                                        <i class="bi bi-geo-alt-fill me-2"></i> I'm Here
                                    </button>
                                </div>
                            </div>
                            ...
                            <!-- Map Section -->
                            <div class="col-12">
                                <div class="card border-0 map-container-card">
                                    <div class="card-header text-white" style="background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); padding: 16px;">
                                        <div class="d-flex align-items-center">
                                            <i class="bi bi-map fs-5 me-2"></i>
                                            <span class="fw-bold">Route Map</span>
                                        </div>
                                    </div>
                                    <div class="card-body p-0">
                                        <div id="resMap" style="height: 350px; width: 100%;"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-lg-5">
                        <div class="card border-0 h-100" style="box-shadow: 0 10px 30px rgba(0,0,0,0.1); border-radius: 16px; overflow: hidden;">
                            <div class="card-header text-white d-flex align-items-center justify-content-between" style="background: linear-gradient(135deg, #11998e 0%, #38ef7d 100%); padding: 20px;">
                                <div class="d-flex align-items-center">
                                    <i class="bi bi-list-check fs-4 me-2"></i>
                                    <span class="fw-bold">My Active Reservations</span>
                                </div>
                                <span class="badge" style="background: rgba(255,255,255,0.3); color: white; font-size: 1rem; padding: 6px 12px; border-radius: 20px;" id="resCount">0</span>
                            </div>
                            <div class="card-body p-3" style="max-height: 700px; overflow-y: auto; background: #f8f9fa;">
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="fareModalLabel">LTFRB Region XI Official Fare Matrix</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="table-responsive">
                        <table class="table table-sm fare-table">
                            <thead>
                                <tr><th>Distance</th><th>Regular</th><th>Discounted*</th></tr>
                            </thead>
                            <tbody id="fareTableBody"></tbody>
                        </table>
                    </div>
                    <small class="text-muted">*Discount applies to students, seniors (60+), and PWDs with valid ID</small>
                </div>
            </div>
        </div>
    </div>
    ...
    (function(){
       ...
       function drawMap(){
        ...
        // Function to parse MySQL datetime string to local Date object
        function parseMySQLDateTime(dateStr) {
            ...
            try {
                ...
                // Parts: [full, year, month, day, hour, minute, second]
                ...
                if (isNaN(date.getTime())) {
                    console.error('Invalid date parsed:', dateStr, '->', date);
                    return null;
                }
        ...
        // Function to format ETA time display
        // If serverMinutes is provided, use it (more accurate, avoids timezone issues)
        function formatEtaTime(etaTimeStr, serverMinutes) {
            // Prefer server-calculated minutes if available
            if (serverMinutes !== null && serverMinutes !== undefined) {
                const mins = parseInt(serverMinutes);
                if (mins > 0) {
            ...
            try {
                const etaTime = parseMySQLDateTime(etaTimeStr);
                if (!etaTime) {
                    console.error('Could not parse ETA time:', etaTimeStr);
                    return '';
                }
                
                const now = new Date();
                const etaMs = etaTime.getTime() - now.getTime();
                
                // If time has passed or is very close (within 10 seconds), show appropriate message
                if (etaMs <= 0) {
                    return 'Driver is arriving';
                }
                
                // If less than 1 minute, show "Less than 1 min"
                if (etaMs < 60000) {
                    return 'Less than 1 min';
                }
                ...
                console.error('Error formatting ETA time:', e, etaTimeStr);
                return '';
            }
        }
        
        // Function to format wait time display
        function formatWaitTime(hereAtStr) {
            if (!hereAtStr) return '';
            try {
                const hereAt = parseMySQLDateTime(hereAtStr);
                if (!hereAt) {
                    console.error('Could not parse wait time:', hereAtStr);
                    return '';
                }
                
                const now = new Date();
                const waitMs = now.getTime() - hereAt.getTime();
                
                // If negative, the date is in the future (shouldn't happen, but handle it)
                if (waitMs < 0) {
                    console.warn('Wait time is negative (future date):', hereAtStr, 'Current:', now.toISOString());
                    return '0m 0s';
                }
                
                // If wait time is more than 24 hours, something is likely wrong
                if (waitMs > 24 * 60 * 60 * 1000) {
                    console.warn('Wait time is very large:', waitMs, 'ms for date:', hereAtStr);
                }
                ...
                console.error('Error formatting wait time:', e, hereAtStr);
                return '';
            }
        }
        
        // Function to update live timers for ETA and wait time
        function updateLiveTimers() {
            const items = document.querySelectorAll('.reservation-item');
            items.forEach(item => {
                const etaTimeAttr = item.getAttribute('data-eta-time');
                const hereAtAttr = item.getAttribute('data-here-at');
                const boardedAttr = item.getAttribute('data-boarded');
                
                // Update ETA time
                if (etaTimeAttr && !boardedAttr) {
                    const etaEl = item.querySelector('.eta-display');
                    if (etaEl) {
                        // Get server-calculated minutes from data attribute if available
                        const serverMinutes = item.getAttribute('data-eta-minutes');
                        const serverMinutesNum = serverMinutes && serverMinutes !== '' ? parseInt(serverMinutes) : null;
                        const formatted = formatEtaTime(etaTimeAttr, serverMinutesNum);
                        if (formatted) {
                            etaEl.innerHTML = `<i class="bi bi-truck text-info"></i> <strong>ETA:</strong> ${formatted}`;
                            // Update the data attribute if we calculated it client-side (for next update)
                            if (serverMinutesNum === null) {
                                // Recalculate and store for next time
                                const etaTime = parseMySQLDateTime(etaTimeAttr);
                                if (etaTime) {
                                    const now = new Date();
                                    const etaMs = etaTime.getTime() - now.getTime();
                                    const calculatedMinutes = Math.max(0, Math.floor(etaMs / 60000));
                                    item.setAttribute('data-eta-minutes', calculatedMinutes.toString());
                                }
                            }
                        } else {
                            // If no formatted time, show "Driver is arriving"
                            etaEl.innerHTML = `<i class="bi bi-truck text-success"></i> <strong>ETA:</strong> Driver is arriving`;
                        }
                    }
                }
                
                // Update wait time
                if (hereAtAttr && !boardedAttr) {
                    const waitEl = item.querySelector('.wait-display');
                    if (waitEl) {
                        const formatted = formatWaitTime(hereAtAttr);
                        if (formatted) {
                            waitEl.innerHTML = `<i class="bi bi-clock-history text-warning"></i> <strong>Wait:</strong> ${formatted}`;
                        }
                    }
                }
            });
        ...
        function listMy(){
            ...
            .then(r=>r.json()).then(d=>{
                ...
                box.innerHTML = d.reservations.map(r=>{
                    ...
                    // Store timestamps as data attributes for live updates
                    const hasEta = r.eta_time && !r.boarded_at;
                    const hasWait = r.here_at && !r.boarded_at;
                    
                    // Initial display values
                    let waitTxt = '';
                    if (hasWait) {
                        waitTxt = `<div class="mt-2 wait-display"><i class="bi bi-clock-history text-warning"></i> <strong>Wait:</strong> ${formatWaitTime(r.here_at)}</div>`;
                    }
                    
                    let etaTxt = '';
                    if (hasEta) {
                        // Use server-calculated minutes if available (more accurate)
                        const formattedEta = formatEtaTime(r.eta_time, r.eta_minutes_remaining);
                        if (formattedEta) {
                            etaTxt = `<div class="mt-2 eta-display" data-eta-minutes="${r.eta_minutes_remaining || ''}"><i class="bi bi-truck text-info"></i> <strong>ETA:</strong> ${formattedEta}</div>`;
                        }
                    }
                    ...
                    const statusClass = r.status === 'boarded' ? 'status-boarded' : 'status-requested';
                    const payBtn = canPay ? `<div class='mt-3 d-grid'><button class='btn btn-sm' style='background: linear-gradient(135deg, #11998e 0%, #38ef7d 100%); color: white; border: none; border-radius: 8px; padding: 10px; font-weight: 600;' onclick='openPayModal(${r.id}, ${reg||0}, ${disc||0}, ${JSON.stringify(routeName)})'><i class="bi bi-cash-coin me-1"></i> Pay Exact Fare ₱${amt.toFixed(2)}</button></div>` : '';
                    const reservationNumber = r.reservation_number || r.id; // Use per-passenger number if available, fallback to id
                    return `<div class='reservation-item' data-reservation-id='${r.id}' data-eta-time='${r.eta_time || ''}' data-eta-minutes='${r.eta_minutes_remaining || ''}' data-here-at='${r.here_at || ''}' data-boarded='${r.boarded_at || ''}'>
                        <div class='d-flex justify-content-between align-items-start mb-2'>
                            <div>
                                <div class='fw-bold text-dark' style='font-size: 0.95rem;'><i class="bi bi-receipt text-primary"></i> #${reservationNumber}</div>
                            </div>
                            <div class='d-flex gap-1'>
                                <span class='status-badge ${statusClass}'>${r.status}</span>
                                ${isPaid ? "<span class='badge bg-success'>Paid</span>" : ''}
                            </div>
                        </div>
                        <div class='mb-2'>
                            <div class='d-flex align-items-center text-dark' style='font-size: 0.9rem;'>
                                <i class="bi bi-geo-alt text-primary me-1"></i>
                                <span class='fw-semibold'>${r.origin_landmark}</span>
                                <i class="bi bi-arrow-right mx-2 text-muted"></i>
                                <i class="bi bi-flag text-danger me-1"></i>
                                <span class='fw-semibold'>${r.dest_landmark}</span>
                            </div>
                        </div>
                        <div class='small text-muted'>
                            <i class="bi bi-signpost-2"></i> <strong>Distance:</strong> ${r.distance_km||'-'} km
                        </div>
                        ${waitTxt}
                        ${etaTxt}
                        ${payBtn}
                ...
                if(etaCandidate){
                    ...
                    if(key !== lastEtaKey){
                        lastEtaKey = key;
                        // Use server-calculated minutes if available (more accurate, avoids timezone issues)
                        let notificationText = '';
                        if (etaCandidate.eta_minutes_remaining !== null && etaCandidate.eta_minutes_remaining !== undefined) {
                            const mins = parseInt(etaCandidate.eta_minutes_remaining);
                            if (mins > 0) {
                                notificationText = mins === 1 ? '1 minute' : `${mins} minutes`;
                            } else {
                                notificationText = 'a few moments';
                            }
                        } else {
                            // Fallback to client-side calculation
                            const formattedEta = formatEtaTime(etaCandidate.eta_time);
                            if (!formattedEta || formattedEta === 'Arrived') {
                                // Try to calculate minutes from the datetime string
                                const etaTime = parseMySQLDateTime(etaCandidate.eta_time);
                                if (etaTime) {
                                    const now = new Date();
                                    const etaMs = etaTime.getTime() - now.getTime();
                                    const totalMinutes = Math.max(1, Math.ceil(etaMs / 60000));
                                    notificationText = totalMinutes === 1 ? '1 minute' : `${totalMinutes} minutes`;
                                } else {
                                    notificationText = 'soon';
                                }
                            } else {
                                // Parse the formatted ETA to create a cleaner message
                                const match = formattedEta.match(/(\d+)m/);
                                if (match) {
                                    const mins = parseInt(match[1]);
                                    notificationText = mins === 1 ? '1 minute' : `${mins} minutes`;
                                } else if (formattedEta.match(/(\d+)s/)) {
                                    notificationText = 'less than a minute';
                                } else {
                                    notificationText = formattedEta;
                                }
                            }
                        }
                        if(window.Swal && notificationText){ 
                            Swal.fire({
                                icon:'info', 
                                title:'Driver ETA', 
                                text:`Driver arriving in approximately ${notificationText}.`,
                                timer: 5000,
                ...
                // Start live timer updates after rendering
                updateLiveTimers();
            });
        }
        ...
        document.addEventListener('DOMContentLoaded', function(){
            ...
            listMy(); 
            // Refresh reservation list every 8 seconds
            setInterval(listMy, 8000);
            // Update live timers (ETA and wait time) every second
