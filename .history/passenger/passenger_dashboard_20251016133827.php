...
<head>
    ...
    /* Ensure modal overlay works correctly and closes when clicked */
    .modal-backdrop {
    ...
    /* Ensure modal itself always on top */
    .modal {
        position: fixed !important;
    ...
    /* Ensure sidebar and wrapper are not blocking interaction ONLY when modal is open */
    ...
    /* Prevent click blocking over modal */
    .modal-dialog {
        pointer-events: auto !important;
    ...
    /* FORCE modal to be above everything and clickable - FINAL FIX */
    #applyModal {
...
<script>
// Verify Bootstrap is loaded and fix modal interactions
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof bootstrap !== 'undefined') {
        ...
        console.log('🔍 Apply Modal:', applyModal ? 'FOUND' : 'NOT FOUND');
        
        // Count buttons
        const applyButtons = document.querySelectorAll('[data-bs-target="#applyModal"]');
        console.log('🔘 Found', applyButtons.length, 'Apply Now buttons');
        
        // Fix modal interactions when shown
        if (applyModal) {
            applyModal.addEventListener('shown.bs.modal', function (e) {
                console.log('✅ Modal shown event fired');
                
                // Ensure modal and its children are clickable
                const modalDialog = this.querySelector('.modal-dialog');
                const modalContent = this.querySelector('.modal-content');
                const modalBackdrop = document.querySelector('.modal-backdrop');
                
                // Set proper z-index stacking AND pointer-events
                if (modalBackdrop) {
                    modalBackdrop.style.zIndex = '19999';
                    modalBackdrop.style.pointerEvents = 'auto';
                }
                if (modalDialog) {
                    modalDialog.style.position = 'relative';
                    modalDialog.style.zIndex = '20001';
                    modalDialog.style.pointerEvents = 'auto';
                }
                if (modalContent) {
                    modalContent.style.position = 'relative';
                    modalContent.style.zIndex = '20002';
                    modalContent.style.pointerEvents = 'auto';
                }
                
                // Ensure ALL interactive elements are clickable
                const interactiveElements = this.querySelectorAll('input, select, button, textarea, .form-control, .form-select, .btn, label, .btn-close');
                interactiveElements.forEach(element => {
                    element.style.pointerEvents = 'auto';
                    element.style.cursor = 'pointer';
                });
                
                console.log('✅ Modal fully interactive - z-index and pointer-events applied');
                console.log('   Backdrop z-index:', modalBackdrop ? '19999' : 'not found');
                console.log('   Dialog z-index:', modalDialog ? '20001' : 'not found');
                console.log('   Content z-index:', modalContent ? '20002' : 'not found');
                console.log('   Interactive elements enabled:', interactiveElements.length);
            });
        }
    } else {
