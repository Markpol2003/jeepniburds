...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_payment'])) {
    ...
    try {
        ...
        // Now handle payment_details table
        $checkPaymentDetailsTable = "SHOW TABLES LIKE 'payment_details'";
        $tableExists = $conn->query($checkPaymentDetailsTable)->num_rows > 0;
        
        if ($tableExists) {
            // First, delete any existing record with the old receipt number
            if (!empty($paymentInfo['receipt_number'])) {
                $deleteDetailsStmt = $conn->prepare("DELETE FROM payment_details WHERE receipt_number = ?");
                $deleteDetailsStmt->bind_param('s', $paymentInfo['receipt_number']);
                if (!$deleteDetailsStmt->execute()) {
                    throw new Exception('Failed to delete old payment details: ' . $deleteDetailsStmt->error);
                }
            }

            // Then insert new record with the new receipt number
            ...
            if (!$insertDetailsStmt->execute()) {
                throw new Exception('Failed to insert new payment details: ' . $insertDetailsStmt->error);
            }
        }
