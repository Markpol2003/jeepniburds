    ...
    function fetchBoundaries() {
        ...
        .then(data => {
            ...
            if (data.success && data.boundaries.length > 0) {
                ...
                const pendingBoundaries = data.boundaries.filter(b => b.status === 'Pending');
                if (data.success && pendingBoundaries.length > 0) {
                    // DEBUG: Show raw data
                    document.getElementById('boundariesTableContainer').innerHTML = `<pre style='background:#f8f9fa;color:#333;padding:10px;border-radius:6px;'>DEBUG: ` + JSON.stringify(pendingBoundaries, null, 2) + `</pre>`;
                    let html = `
                        <div class='table-responsive'>
                ...
                pendingBoundaries.forEach(b => {
                    console.log('Boundary row:', b);
                    // REMOVED: if (!b.reference_number || b.reference_number.trim() === '' || b.reference_number === 'null') return;
