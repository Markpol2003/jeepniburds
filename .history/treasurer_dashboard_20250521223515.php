...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_payment'])) {
    ...
    try {
        ...
        $updatePaymentStmt = $conn->prepare("UPDATE membership_payments SET status = 'Confirmed', receipt_number = ? WHERE id = ?");
        ...
        // Store receipt in user_receipts table
        ...
        $insertReceiptStmt->bind_param('iissss', 
            $paymentInfo['user_id'],
            $paymentId,
            $receiptNumber,
            $paymentInfo['amount'],
        ...
        $(document).ready(function () {
            ...
            $('form').on('submit', function(e) {
                if ($(this).find('button[name="confirm_payment"]').length) {
                    e.preventDefault();
                    const form = this;
                    const paymentId = $(form).find('input[name="payment_id"]').val();
                            ...
                            .then(data => {
                                if (data.success) {
                                    Swal.fire({
                                        icon: 'success',
                                        title: 'Success!',
                                        text: data.message || 'Payment confirmed successfully',
                                        confirmButtonColor: '#198754'
                                    }).then(() => {
