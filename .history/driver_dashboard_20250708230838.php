...
<div class="d-flex">
        ...
        <ul class="nav flex-column">
            ...
            <li class="nav-item mb-2">
                <a class="nav-link text-white <?= $page === 'collect_fares' ? 'fw-bold' : '' ?>" href="?page=collect_fares">
                    <i class="bi bi-cash-coin me-2"></i> Collect Fares
                </a>
            </li>
            <!-- Profile Link -->
            <li class="nav-item mb-2">
    ...
    <div class="content container" style="margin-left: 250px;">
        <?php if ($page === 'dashboard'): ?>
            ...
            <!-- Orientation Requests Section -->
            <?php
            // Fetch all pending orientation requests (for all users)
            $orientationReqQuery = "
            ...
            ";
            $orientationReqResult = $conn->query($orientationReqQuery);
            ?>
            <div class="card shadow-sm mb-4">
                <div class="card-header bg-warning text-dark d-flex align-items-center">
                    <i class="bi bi-bell-fill me-2"></i>
                    <h5 class="mb-0">Pending Orientation Requests</h5>
                </div>
                <div class="card-body">
                    <?php if ($orientationReqResult && $orientationReqResult->num_rows > 0): ?>
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped mb-0">
                                ...
                                <tbody>
                                    <?php $i = 1; while ($row = $orientationReqResult->fetch_assoc()): ?>
                                        <tr>
                                            <td><?= $i++; ?></td>
                    ...
                    <?php else: ?>
                        <div class="alert alert-info text-center mb-0">
                            <i class="bi bi-info-circle"></i> No pending orientation requests at the moment.
                        </div>
                    <?php endif; ?>
                </div>
            </div>

            <!-- Recent Fare Payments Section -->
            <?php
            // Fetch recent fare payments for the assigned route
            $recentFares = [];
            if (!empty($assignedJeepney['route'])) {
                $route = $assignedJeepney['route'];
                $fareQuery = "SELECT fp.*, u.firstName, u.lastName FROM fare_payments fp JOIN users u ON fp.passenger_id = u.id WHERE fp.route = ? ORDER BY fp.paid_at DESC LIMIT 10";
                $fareStmt = $conn->prepare($fareQuery);
                $fareStmt->bind_param('s', $route);
                $fareStmt->execute();
            ...
            ?>
            <div class="card shadow-sm mb-4" id="recentFaresCard">
                <div class="card-header bg-success text-white d-flex align-items-center">
                    <i class="bi bi-cash-coin me-2"></i>
                    <h5 class="mb-0">Recent Fare Payments (Route: <?= htmlspecialchars($assignedJeepney['route'] ?? 'N/A'); ?>)</h5>
                </div>
                <div class="card-body" id="recentFaresBody">
                    <?php if (!empty($recentFares)): ?>
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped mb-0" id="recentFaresTable">
                                <thead class="table-light">
                                    <tr>
                                        <th>#</th>
                                        <th>Passenger Name</th>
                                ...
                                <tbody>
                                    <?php $i = 1; foreach ($recentFares as $fare): ?>
                                        <tr data-receipt="<?= htmlspecialchars($fare['receipt_number']); ?>">
                                            <td><?= $i++; ?></td>
                                            <td><?= htmlspecialchars($fare['firstName'] . ' ' . $fare['lastName']); ?></td>
                                            <td>₱<?= number_format($fare['amount'], 2); ?></td>
                                            <td><?= htmlspecialchars($fare['payment_method']); ?></td>
                                            <td><?= date('M d, Y h:i A', strtotime($fare['paid_at'])); ?></td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
                    ...
                    <?php else: ?>
                        <div class="alert alert-info text-center mb-0" id="noFaresAlert">
                            <i class="bi bi-info-circle"></i> No fare payments yet for this route.
                        </div>
                    <?php endif; ?>
                </div>
            </div>
            <script>
            <?php if (!empty($assignedJeepney['route'])): ?>
            let lastFareReceipts = Array.from(document.querySelectorAll('#recentFaresTable tbody tr')).map(tr => tr.getAttribute('data-receipt'));
            function fetchRecentFares() {
                fetch('pay_fare.php', {
                    method: 'POST',
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        const tbody = document.querySelector('#recentFaresTable tbody');
                        if (fares.length === 0) {
                            document.getElementById('noFaresAlert').style.display = '';
                            tbody.innerHTML = '';
                            lastFareReceipts = [];
                            return;
                        } else {
                            document.getElementById('noFaresAlert').style.display = 'none';
                        }
                        // Detect new fares
                        const newReceipts = fares.map(f => f.receipt_number);
                        const newFares = fares.filter(f => !lastFareReceipts.includes(f.receipt_number));
                        // Update table
                        tbody.innerHTML = fares.map((fare, i) => `
                            <tr data-receipt="${fare.receipt_number}">
                                <td>${i+1}</td>
                                <td>${fare.passenger}</td>
                                <td>₱${parseFloat(fare.amount).toFixed(2)}</td>
                                <td>${fare.payment_method}</td>
                                <td>${new Date(fare.paid_at).toLocaleString()}</td>
                            </tr>
                        `).join('');
                        ...
                        newFares.forEach(fare => {
                            Swal.fire({
                                ...
                                position: 'top-end',
                                icon: 'success',
                                title: `Fare paid by ${fare.passenger}: ₱${parseFloat(fare.amount).toFixed(2)} (${fare.payment_method})`,
                                showConfirmButton: false,
                                timer: 4000
                            });
                        });
                        lastFareReceipts = newReceipts;
                    }
                });
            }
            setInterval(fetchRecentFares, 10000);
            <?php endif; ?>
            </script>

            <!-- Recent Boarding and Fare Events Section -->
            <?php
            $recentEvents = [];
            if (!empty($assignedJeepney['route'])) {
                $route = $assignedJeepney['route'];
                // Fetch last 10 boarding events
                $boardingQuery = "SELECT b.passenger_id, u.firstName, u.lastName, b.boarded_at AS event_time, 'Boarded' AS event_type FROM boarding_events b JOIN users u ON b.passenger_id = u.id WHERE b.route = ?";
                $boardingStmt = $conn->prepare($boardingQuery);
                $boardingStmt->bind_param('s', $route);
                $boardingStmt->execute();
                $boardingResult = $boardingStmt->get_result();
                while ($row = $boardingResult->fetch_assoc()) {
                    $recentEvents[] = [
                        'type' => 'Boarded',
                ...
                // Fetch last 10 fare payments
                $fareQuery = "SELECT fp.passenger_id, u.firstName, u.lastName, fp.paid_at AS event_time, 'Paid Fare' AS event_type, fp.amount, fp.payment_method FROM fare_payments fp JOIN users u ON fp.passenger_id = u.id WHERE fp.route = ?";
                $fareStmt = $conn->prepare($fareQuery);
                $fareStmt->bind_param('s', $route);
                $fareStmt->execute();
                $fareResult = $fareStmt->get_result();
                ...
                // Sort all events by time descending, limit to 10
            ...
            <div class="card shadow-sm mb-4">
                <div class="card-header bg-primary text-white d-flex align-items-center">
                    ...
                    <h5 class="mb-0">Recent Boarding & Fare Events (Route: <?= htmlspecialchars($assignedJeepney['route'] ?? 'N/A'); ?>)</h5>
                </div>
                <div class="card-body">
                    <?php if (!empty($recentEvents)): ?>
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped mb-0">
                                ...
                                <tbody>
                                    <?php $i = 1; foreach ($recentEvents as $event): ?>
                                        <tr>
                                            <td><?= $i++; ?></td>
                                            <td>
                                                <?php if ($event['type'] === 'Boarded'): ?>
                                                    <span class="badge bg-info"><i class="bi bi-person-walking"></i> Boarded</span>
                                                <?php else: ?>
                                                    <span class="badge bg-success"><i class="bi bi-cash"></i> Paid Fare</span>
                                                <?php endif; ?>
                                            </td>
                                            <td><?= htmlspecialchars($event['passenger']); ?></td>
                                            <td><?= $event['amount'] !== null ? '₱' . number_format($event['amount'], 2) : '-'; ?></td>
                                            <td><?= $event['method'] !== null ? htmlspecialchars($event['method']) : '-'; ?></td>
                                            <td><?= date('M d, Y h:i A', strtotime($event['time'])); ?></td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
                    ...
                    <?php else: ?>
                        <div class="alert alert-info text-center mb-0">
                            <i class="bi bi-info-circle"></i> No boarding or fare events yet for this route.
                        </div>
                    <?php endif; ?>
                </div>
            </div>
        <?php elseif ($page === 'collect_fares' && $assignedJeepney): ?>
            <!-- Collect Fares Section -->
            <div class="card shadow-sm mb-4">
                <div class="card-header bg-info text-white d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">💸 Collect Fares</h5>
                    <button class="btn btn-light btn-sm" id="fetchFaresBtn"><i class="bi bi-arrow-repeat"></i> Refresh</button>
                </div>
                <div class="card-body">
                    <div id="faresLoading" class="text-center text-muted mb-2" style="display:none;">
                        <div class="spinner-border text-info" role="status"></div>
                        <div>Loading fares...</div>
                    </div>
                    <div id="faresTableContainer"></div>
                </div>
            </div>
            <script>
            ...
            function fetchFares() {
                ...
                fetch('pay_fare.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ action: 'list', route })
                })
                .then(res => res.json())
                .then(data => {
                    document.getElementById('faresLoading').style.display = 'none';
                    if (data.success && data.fares.length > 0) {
                        let html = `<div class='table-responsive'><table class='table table-bordered table-striped'><thead><tr><th>Passenger</th><th>Amount</th><th>Method</th><th>Date</th><th>Receipt</th><th>Status</th><th>Action</th></tr></thead><tbody>`;
                        data.fares.forEach(fare => {
                            ...
                            html += `<tr>
                                ...
                                <td>${fare.payment_method}</td>
                                <td>${fare.paid_at}</td>
                                <td>${fare.receipt_number}</td>
                                <td><span class='badge ${isCollected ? 'bg-success' : 'bg-warning text-dark'}'>${isCollected ? 'Collected' : 'Paid'}</span></td>
                                <td>`;
                            if (!isCollected) {
                                html += `<button class='btn btn-sm btn-success' onclick=\"confirmCollected('${fare.receipt_number}', this)\">Confirm Collected</button>`;
                            } else {
                                html += '-';
                            }
                            html += `</td></tr>`;
                        ...
                        document.getElementById('faresTableContainer').innerHTML = '<div class="alert alert-info">No fares collected yet for this route.</div>';
                    }
                })
                .catch(() => {
                    document.getElementById('faresLoading').style.display = 'none';
                    document.getElementById('faresTableContainer').innerHTML = '<div class="alert alert-danger">Failed to load fares.</div>';
                });
            }
            function confirmCollected(receiptNumber, btn) {
                ...
                .then(data => {
                    if (data.success) {
                        btn.closest('tr').querySelector('td:nth-child(6) span').className = 'badge bg-success';
                        btn.closest('tr').querySelector('td:nth-child(6) span').textContent = 'Collected';
                        btn.replaceWith('-');
                    } else {
                        btn.disabled = false;
        ...
        <?php if ($page === 'pay_boundary' && $assignedJeepney): ?>
            ...
            <div class="card shadow-sm mb-4">
                <div class="card-header bg-warning text-dark">
                    <h5 class="mb-0">Pay Boundary to Operator</h5>
                </div>
                <div class="card-body">
                    <form id="boundaryForm">
                        <div class="mb-3">
                            <label class="form-label">Amount</label>
                            <input type="number" class="form-control" name="amount" id="boundaryAmount" value="500" min="0" required>
                        </div>
                        <div class="mb-3">
                        ...
                        <button type="submit" class="btn btn-warning w-100">Pay Boundary</button>
                    </form>
            ...
            document.getElementById('boundaryForm').addEventListener('submit', function(e) {
                ...
                fetch('pay_boundary.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ driver_id, operator_id, jeepney_id, amount, payment_method })
                })
                .then(res => res.json())
                .then(data => {
                    if (data.success) {
                        document.getElementById('boundaryReceipt').innerHTML = `<div class='alert alert-success'><strong>Boundary Paid!</strong><br>Amount: ₱${data.receipt.amount}<br>Method: ${data.receipt.payment_method}<br>Date: ${data.receipt.date}</div>`;
                    } else {
                        document.getElementById('boundaryReceipt').innerHTML = `<div class='alert alert-danger'>${data.message || 'Payment failed.'}</div>`;
                    }
                })
                .catch(() => {
                    document.getElementById('boundaryReceipt').innerHTML = `<div class='alert alert-danger'>Payment failed.</div>`;
                });
            });
            </script>
        <?php endif; ?>
