...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'send_receipt') {
    ...
    try {
        ...
        $paymentMethod = $payment['payment_method'] ?? 'cash'; // Changed from 'method' to 'payment_method'
        $paymentDate = $payment['created_at'] ?? date('Y-m-d H:i:s'); // Changed from 'payment_date' to 'created_at'
        ...
        $to = $payment['email'];
        $subject = 'JeepniGo - Payment Receipt';
        $message = "
        <html>
        <head>
            <style>
        ...
        <body>
            <div class='receipt'>
                <div class='header'>
                    <h2>JeepniGo Payment Receipt</h2>
                    <p>Receipt Number: " . $payment['receipt_number'] . "</p>
                </div>
                <div class='details'>
                    <p><strong>Date:</strong> " . date('F d, Y', strtotime($paymentDate)) . "</p>
                ...
                <div class='footer'>
                    ...
                    <p>This receipt serves as proof of your payment to JeepniGo.</p>
                </div>
            </div>
        </body>
        </html>";
        ...
        $headers .= "From: JeepniGo <noreply@jeepnigo.com>" . "\r\n";
        ...
        function sendReceipt(paymentId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    .then(data => {
                        if (data.success) {
                            // Update the UI to show the receipt section
                            ...
                            if (row) {
                                ...
                                actionCell.innerHTML = `
                                    <div class="flex items-center gap-2">
                                        ...
                                        <button onclick="viewReceipt(${paymentId})" 
                                                class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-200">
                                            <i class="bi bi-eye mr-2"></i>
                                            View Receipt
                                        </button>
                            ...
                            Swal.fire({
                                icon: 'success',
                                title: 'Receipt Sent!',
                                text: 'The receipt has been sent to the member\'s email and is now available for viewing.',
                                confirmButtonColor: '#4f46e5'
                            });
                        } else {
