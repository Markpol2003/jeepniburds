...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'send_receipt') {
    ...
    try {
        ...
        $paymentDate = $payment['created_at'] ?? date('Y-m-d H:i:s');
                    ...
                    .then(data => {
                        if (data.success) {
                            Swal.fire({
                                ...
                                // Add view receipt button below the payment form
                                const paymentForm = document.querySelector('.payment-form');
                                if (paymentForm) {
                                    const viewReceiptBtn = document.createElement('button');
                                    viewReceiptBtn.className = 'btn btn-primary mt-3 w-100';
                                    viewReceiptBtn.innerHTML = '<i class="bi bi-eye"></i> View Receipt';
                                    viewReceiptBtn.onclick = () => viewReceipt(paymentId);
                                    paymentForm.appendChild(viewReceiptBtn);
                                }
                            });
                        } else {
