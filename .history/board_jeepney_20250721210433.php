...
if (isset($data['action']) && $data['action'] === 'list' && isset($data['route'])) {
    $route = $data['route'];
    $stmt = $conn->prepare("SELECT b.passenger_id, u.firstName, u.lastName, b.event_time, 'Boarded' AS event_type FROM boarding_events b JOIN users u ON b.passenger_id = u.id WHERE b.route = ? ORDER BY b.event_time DESC");
