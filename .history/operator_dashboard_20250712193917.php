...
<body>
    ...
    <div class="d-flex">
        <!-- Sidebar -->
        <div class="sidebar bg-dark text-white p-3" style="min-width: 220px; position: fixed; height: 100vh;">
            ...
            <ul class="nav flex-column">
                ...
                <!-- Assign Jeepney Link -->
                <li class="nav-item mb-2">
                    <a class="nav-link text-white <?= $page === 'assignjeepney' ? 'fw-bold' : '' ?>" href="?page=assignjeepney">
                        <i class="bi bi-truck-front me-2"></i> Assign Jeepney
                    </a>
                </li>

                <!-- Collect Boundaries Link -->
                <li><a href="?page=collect_boundaries" class="<?= $page === 'collect_boundaries' ? 'active' : '' ?>"><i class="bi bi-cash-coin me-2"></i> Collect Boundaries</a></li>
        ...
        <!-- Main Content -->
        <div class="content-wrapper" style="margin-left: 220px; width: 100%;">
            <!-- Navbar -->
...
<?php if ($page === 'collect_boundaries'): ?>
    ...
    <div class="card shadow-sm mb-4">
        <div class="card-header bg-info text-white d-flex justify-content-between align-items-center">
            <h5 class="mb-0">Collect Boundaries</h5>
            <div>
                <button class="btn btn-light btn-sm me-2" id="testBoundaryBtn"><i class="bi bi-bug"></i> Test</button>
                <button class="btn btn-light btn-sm" id="fetchBoundariesBtn"><i class="bi bi-arrow-repeat"></i> Refresh</button>
            </div>
        </div>
        <div class="card-body">
            <div id="boundariesLoading" class="text-center text-muted mb-2" style="display:none;">
    ...
    console.log('Current operator_id:', operator_id); // Debug log
    function fetchBoundaries() {
        ...
        .then(data => {
            ...
            console.log('Boundary data received:', data); // Debug log
            console.log('Operator ID:', operator_id); // Debug log
            console.log('Request sent:', { action: 'list', operator_id }); // Debug log
            if (data.success && data.boundaries.length > 0) {
                let html = `<div class='table-responsive'><table class='table table-bordered table-striped'><thead><tr><th>Driver</th><th>Jeepney</th><th>Route</th><th>Amount</th><th>Method</th><th>Date</th><th>Status</th><th>Action</th></tr></thead><tbody>`;
                data.boundaries.forEach(b => {
                    const isCollected = b.status === 'Collected';
                    html += `<tr>
                        <td>${b.driver}</td>
                        <td>${b.jeepney}</td>
                        <td><span class='badge bg-info'>${b.route || 'N/A'}</span></td>
                        <td>₱${b.amount}</td>
                        <td>${b.payment_method}</td>
                        <td>${b.paid_at}</td>
                        <td><span class='badge ${isCollected ? 'bg-success' : 'bg-warning text-dark'}'>${isCollected ? 'Collected' : 'Pending'}</span></td>
                        <td>`;
                    if (!isCollected) {
                        html += `<button class='btn btn-sm btn-success' onclick=\"confirmBoundary(${b.id}, this)\">Confirm Collected</button>`;
                    } else {
                        html += '-';
                    }
                    html += `</td></tr>`;
                ...
                document.getElementById('boundariesTableContainer').innerHTML = '<div class="alert alert-info">No boundary payments found.</div>';
            }
        })
        .catch(() => {
            document.getElementById('boundariesLoading').style.display = 'none';
            document.getElementById('boundariesTableContainer').innerHTML = '<div class="alert alert-danger">Failed to load boundaries.</div>';
        });
    }
    ...
    function testBoundarySystem() {
        console.log('Testing boundary system...');
        console.log('Current operator_id:', operator_id);
    ...
    document.getElementById('testBoundaryBtn').addEventListener('click', testBoundarySystem);
