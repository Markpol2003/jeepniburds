...
<nav class="mobile-navbar sticky top-0 z-50 text-white shadow animated-gradient">
  ...
  <div id="mobileMenu" class="hidden bg-gradient-to-r from-indigo-600 to-purple-600 border-t border-white/10">
    <div class="px-4 py-2 space-y-1">
      ...
      <a href="?section=performance" class="block px-4 py-3 rounded-md hover:bg-white/10 text-white text-sm font-semibold flex items-center gap-3">
        <i class="bi bi-graph-up-arrow text-lg"></i> Performance
      </a>
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="h-16 flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        ...
        <a href="?section=performance" class="px-3 py-1.5 rounded-md hover:bg-indigo-500 text-white text-sm font-semibold flex items-center gap-2"><i class="bi bi-graph-up-arrow text-lg"></i> Performance</a>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <?php elseif ($section === 'performance'): ?>
      <div class="mb-6">
        <h2 class="text-2xl font-bold text-slate-800 flex items-center gap-2"><i class="bi bi-graph-up-arrow text-indigo-600"></i> System Performance Monitoring</h2>
        <p class="text-slate-600 mt-1">Monitor waiting times, queue metrics, and system efficiency</p>
      </div>
      
      <!-- Queue Theory Metrics -->
      <div class="bg-white rounded-2xl border p-5 shadow mb-6">
        <h3 class="text-xl font-semibold text-slate-800 mb-4 flex items-center gap-2"><i class="bi bi-calculator text-indigo-600"></i> Queue Theory Metrics (Today)</h3>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div class="border-l-4 border-indigo-500 p-4 rounded-lg bg-slate-50">
            <div class="text-slate-500 text-sm">Arrival Rate (λ)</div>
            <div class="text-3xl font-bold text-indigo-600 mt-1"><?= $todayMetrics['lambda'] ?? 0 ?></div>
            <div class="text-xs text-slate-500 mt-1">reservations/hour</div>
          </div>
          <div class="border-l-4 border-indigo-500 p-4 rounded-lg bg-slate-50">
            <div class="text-slate-500 text-sm">Service Rate (μ)</div>
            <div class="text-3xl font-bold text-indigo-600 mt-1"><?= $todayMetrics['mu'] ?? 0 ?></div>
            <div class="text-xs text-slate-500 mt-1">services/hour</div>
          </div>
          <div class="border-l-4 border-indigo-500 p-4 rounded-lg bg-slate-50">
            <div class="text-slate-500 text-sm">Utilization (ρ)</div>
            <div class="text-3xl font-bold text-indigo-600 mt-1"><?= $todayMetrics['rho'] ?? 0 ?>%</div>
            <div class="text-xs text-slate-500 mt-1">server utilization</div>
          </div>
          <div class="border-l-4 border-indigo-500 p-4 rounded-lg bg-slate-50">
            <div class="text-slate-500 text-sm">Avg Wait Time (Wq)</div>
            <div class="text-3xl font-bold text-indigo-600 mt-1"><?= $todayMetrics['wq'] ?? 0 ?></div>
      ...
      <!-- Comparison Table -->
      <div class="bg-white rounded-2xl border p-5 shadow mb-6">
        <h3 class="text-xl font-semibold text-slate-800 mb-4 flex items-center gap-2"><i class="bi bi-bar-chart text-green-600"></i> Waiting Time Comparison: With vs Without JeepniGo</h3>
        <div class="overflow-x-auto">
          <table class="min-w-full text-left text-sm">
            <thead class="bg-slate-100 text-slate-700 font-semibold">
              <tr>
                <th class="px-4 py-3">Traffic Scenario</th>
                <th class="px-4 py-3 text-end">Without JeepniGo (mins)</th>
                <th class="px-4 py-3 text-end">With JeepniGo (mins)</th>
                <th class="px-4 py-3 text-end">Improvement</th>
                <th class="px-4 py-3 text-end">Trip Count</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-slate-100">
              ...
              foreach ($trafficScenarios as $scenario):
                ...
                $improvementClass = $improvement >= 50 ? 'text-green-600' : ($improvement >= 30 ? 'text-yellow-600' : 'text-blue-600');
              ?>
              <tr>
                <td class="px-4 py-3 font-medium text-slate-900"><?= $scenario ?></td>
                <td class="px-4 py-3 text-end text-red-600 font-semibold"><?= $without ?></td>
                <td class="px-4 py-3 text-end text-green-600 font-semibold"><?= $with ?></td>
                <td class="px-4 py-3 text-end <?= $improvementClass ?> font-semibold"><?= $improvement ?>%</td>
                <td class="px-4 py-3 text-end text-slate-500"><?= $tripCount ?></td>
              </tr>
              <?php endforeach; ?>
            </tbody>
          </table>
        ...
        <div class="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
          <div class="flex items-start gap-2 text-blue-800">
            ...
            <div class="text-sm"><strong>Note:</strong> Data based on last 30 days. Without JeepniGo times are estimated based on industry benchmarks and system improvements.</div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-2xl border p-5 shadow">
        <h3 class="text-xl font-semibold text-slate-800 mb-4 flex items-center gap-2"><i class="bi bi-table text-blue-600"></i> Operational Conditions Analysis</h3>
        <div class="overflow-x-auto">
          <table class="min-w-full text-left text-sm border-collapse border border-slate-300">
            <thead class="bg-slate-100 text-slate-700 font-semibold">
              <tr>
                <th class="px-4 py-3 border border-slate-300">Case</th>
                <th class="px-4 py-3 border border-slate-300 text-end">Arrival Rate (λ)</th>
                <th class="px-4 py-3 border border-slate-300 text-end">Service Rate (μ)</th>
                <th class="px-4 py-3 border border-slate-300 text-end">Utilization (ρ)</th>
                <th class="px-4 py-3 border border-slate-300 text-end">Wait Time (Wq) mins</th>
                <th class="px-4 py-3 border border-slate-300">Analysis</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-slate-100">
              <tr>
                <td class="px-4 py-3 border border-slate-300 font-medium"><strong>Case 1</strong></td>
                <td class="px-4 py-3 border border-slate-300 text-end">2</td>
                <td class="px-4 py-3 border border-slate-300 text-end">5</td>
                <td class="px-4 py-3 border border-slate-300 text-end">40%</td>
                <td class="px-4 py-3 border border-slate-300 text-end text-green-600 font-semibold">0.083</td>
                <td class="px-4 py-3 border border-slate-300">Low congestion, fast service</td>
              </tr>
              <tr>
                <td class="px-4 py-3 border border-slate-300 font-medium"><strong>Case 2</strong></td>
                <td class="px-4 py-3 border border-slate-300 text-end">3</td>
                <td class="px-4 py-3 border border-slate-300 text-end">6</td>
                <td class="px-4 py-3 border border-slate-300 text-end">50%</td>
                <td class="px-4 py-3 border border-slate-300 text-end text-yellow-600 font-semibold">0.143</td>
                <td class="px-4 py-3 border border-slate-300">Moderate utilization, increased wait</td>
              </tr>
              <tr>
                <td class="px-4 py-3 border border-slate-300 font-medium"><strong>Case 3</strong></td>
                <td class="px-4 py-3 border border-slate-300 text-end">4</td>
                <td class="px-4 py-3 border border-slate-300 text-end">8</td>
                <td class="px-4 py-3 border border-slate-300 text-end">50%</td>
                <td class="px-4 py-3 border border-slate-300 text-end text-yellow-600 font-semibold">0.125</td>
                <td class="px-4 py-3 border border-slate-300">Balanced system performance</td>
              </tr>
              <tr>
                <td class="px-4 py-3 border border-slate-300 font-medium"><strong>Case 4</strong></td>
                <td class="px-4 py-3 border border-slate-300 text-end">1</td>
                <td class="px-4 py-3 border border-slate-300 text-end">4</td>
                <td class="px-4 py-3 border border-slate-300 text-end">25%</td>
                <td class="px-4 py-3 border border-slate-300 text-end text-green-600 font-semibold">0.050</td>
                <td class="px-4 py-3 border border-slate-300">Low demand, optimal service</td>
              </tr>
              <tr class="bg-indigo-50">
                <td class="px-4 py-3 border border-slate-300 font-medium"><strong>Current System</strong></td>
                <td class="px-4 py-3 border border-slate-300 text-end font-semibold"><?= $todayMetrics['lambda'] ?? 0 ?></td>
                <td class="px-4 py-3 border border-slate-300 text-end font-semibold"><?= $todayMetrics['mu'] ?? 0 ?></td>
                <td class="px-4 py-3 border border-slate-300 text-end font-semibold"><?= $todayMetrics['rho'] ?? 0 ?>%</td>
                <td class="px-4 py-3 border border-slate-300 text-end font-bold text-indigo-600"><?= $todayMetrics['wq'] ?? 0 ?></td>
                <td class="px-4 py-3 border border-slate-300">Real-time performance metrics</td>
              </tr>
            </tbody>
          </table>
        </div>
