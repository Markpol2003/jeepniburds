...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_orientation'])) {
    ...
    // Derive mode based on provided location/link
    $mode = !empty($link) ? 'Online' : 'In-Person';
...
$profilePath = '../uploads/profile_' . intval($userId) . '.jpg';
    ...
    <div class="col">
        <div class="card text-center h-100">
            <div class="card-body">
                ...
                <p class="card-text">Monitor waiting times, queue metrics, and system efficiency.</p>
                <a href="waiting_time_monitor.php" class="btn btn-info">View Metrics</a>
            </div>
        </div>
    </div>
</div>
...
<?php if (isset($_GET['page']) && $_GET['page'] === 'cooperative_funds'): ?>
    ...
    <div class="container mt-4">
        <div class="card shadow-sm mb-4">
            ...
            <div class="card-body">
                <div id="coopFundsLoading" class="text-center text-muted mb-3" style="display:none;">
                    <div class="spinner-border text-info" role="status"></div>
                    <div>Loading cooperative fund payments...</div>
                </div>
    ...
    function fetchCoopFunds() {
        ...
        .then(data => {
            ...
            if (data.success && data.funds && data.funds.length > 0) {
                let html = `<div class='table-responsive'><table class='table table-bordered'><thead><tr><th>ID</th><th>Member</th><th>Amount</th><th>Method</th><th>Date</th><th>Status</th><th>Action</th></tr></thead><tbody>`;
                data.funds.forEach(f => {
                    html += `<tr><td>${f.id}</td><td>${f.member}</td><td>₱${parseFloat(f.amount).toLocaleString()}</td><td>${f.payment_method}</td><td>${f.paid_at ? new Date(f.paid_at).toLocaleDateString() : ''}</td><td><span class='badge bg-${f.status === 'Confirmed' ? 'success' : 'warning'}'>${f.status}</span></td>`;
                    if (f.status === 'Pending') {
                        html += `<td><button class='btn btn-success btn-lg fw-bold d-flex align-items-center gap-2' onclick='confirmCoopFund(${f.id}, this)'><i class="bi bi-check-circle-fill fs-5"></i> Confirm Payment</button></td>`;
                    } else {
                        html += `<td><span class='text-muted'>Confirmed</span></td>`;
                    }
                    html += `</tr>`;
                });
                ...
                document.getElementById('coopFundsTableContainer').innerHTML = `<div class='alert alert-info text-center mb-0'>No cooperative fund payments found.</div>`;
            }
        });
    }
    function confirmCoopFund(id, btn) {
        Swal.fire({
            ...
            text: 'Are you sure you want to confirm this cooperative fund payment?',
            icon: 'question',
                ...
                .then(data => {
                    if (data.success) {
                        Swal.fire({
                            icon: 'success',
                            title: 'Confirmed!',
                            text: 'The cooperative fund payment has been confirmed.',
                            confirmButtonColor: '#198754',
                            timer: 2000,
                            showConfirmButton: false
                        });
    ...
    document.addEventListener('DOMContentLoaded', fetchCoopFunds);
    </script>
<?php endif; ?>
