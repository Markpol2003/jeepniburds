...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['application_id'], $_POST['action'])) {
    ...
    $getStmt = $conn->prepare('SELECT * FROM cooperative_applications WHERE id = ?');
    $getStmt->bind_param('i', $applicationId);
    $getStmt->execute();
    $app = $getStmt->get_result()->fetch_assoc();
    $getStmt->close();
    ...
    if ($app) {
        if ($action === 'Approve') {
            ...
            // Optionally elevate contact email to manager
...
<head>
    ...
    <title>Admin Dashboard | Cooperative Applications</title>
...
<div class="container py-4">
    ...
    <div class="card shadow-sm">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <span class="badge text-bg-info">Latest</span>
        </div>
        <div class="card-body">
                    ...
                    <?php if ($applications && $applications->num_rows > 0): ?>
                        <?php $i = 1; while ($row = $applications->fetch_assoc()): ?>
                            <tr class="text-center">
                                ...
                                <td>
                                    <a class="btn btn-sm btn-outline-primary" target="_blank" href="<?= htmlspecialchars($row['certificate']); ?>">
...
// Ensure table exists and fetch cooperative applications
$conn->query("CREATE TABLE IF NOT EXISTS cooperative_applications (
    id INT AUTO_INCREMENT PRIMARY KEY,
    user_id INT NOT NULL,
    cooperative_name VARCHAR(255) NOT NULL,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['application_id'])) {
    ...
    if ($action === 'Approve') {
        if ($application) {
            ...
            $updateAppStmt->bind_param("si", $status, $applicationId);
            $updateAppStmt->execute();
            $updateAppStmt->close();

            // Optionally promote contact email to manager userType
