...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_payment'])) {
    ...
    try {
        ...
        $userQuery = "SELECT u.id, u.firstName, u.lastName, u.userType, u.email, p.receipt_number 
        ...
        $newReceiptNumber = 'JEEP-' . date('Ymd') . '-' . str_pad($paymentId, 4, '0', STR_PAD_LEFT);

        // First update payment_details table to avoid foreign key constraint
        if (!empty($userInfo['receipt_number'])) {
            $updateDetailsStmt = $conn->prepare("UPDATE payment_details SET receipt_number = ? WHERE receipt_number = ?");
            $updateDetailsStmt->bind_param('ss', $newReceiptNumber, $userInfo['receipt_number']);
            $updateDetailsStmt->execute();
        }

        // Then update membership_payments table
        $stmt = $conn->prepare("UPDATE membership_payments SET status = 'Confirmed', receipt_number = ? WHERE id = ?");
        $stmt->bind_param('si', $newReceiptNumber, $paymentId);
        $stmt->execute();
    ...
    <script>
        ...
        document.querySelectorAll('form[method="POST"]').forEach(form => {
            form.addEventListener('submit', async function(e) {
                if (this.querySelector('button[name="confirm_payment"]')) {
                    e.preventDefault();
