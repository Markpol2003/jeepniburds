...
<body>
    ...
    <div class="d-flex">
        ...
        <!-- Main Content -->
        <div class="content-wrapper" style="margin-left: 280px; width: 100%;">
            <!-- Navbar -->
            <div class="d-flex align-items-center mb-4" style="padding: 20px 30px 0;">
...
<?php if ($page === 'collect_boundaries'): ?>
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                .catch(error => {
                    ...
                    debugDiv.innerHTML = `<strong>Debug:</strong> ${error.message}`;
                    btn.closest('td').appendChild(debugDiv);
                });
            }
        });
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (headerDiv) {
            headerDiv.appendChild(debugBtn);
        }
    });
    </script>
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="payment-instructions">
                    <div class="instructions-header">
                        <i class="bi bi-info-circle-fill"></i>
                        <h6>Cooperative Fund Payment Instructions</h6>
                    </div>
                    <div class="instructions-content">
                        <div class="amount-info">
                            <span class="label">Amount:</span>
            ...
            <div class="col-md-6">
                <form id="cooperativePaymentForm" class="payment-form" novalidate>
                    <div class="form-group">
                        <label for="coop_amount">Amount</label>
                        <div class="amount-input">
                            <span class="currency-symbol">₱</span>
                            <input type="number" id="coop_amount" name="coop_amount" value="500" readonly>
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="cooperative_payment_method">Payment Method</label>
                        <select id="cooperative_payment_method" name="cooperative_payment_method" required onchange="showCoopPaymentFields()">
                            <option value="">Choose payment method</option>
                            <option value="gcash">GCash</option>
                            <option value="bank">Bank Transfer</option>
                            <option value="cash">Cash Payment</option>
                    ...
                    <div id="coopGcashFields" class="payment-fields d-none">
                        <div class="form-group">
                            <label for="gcash_number">GCash Number</label>
                            <input type="text" id="gcash_number" name="gcash_number" pattern="[0-9]{11}" placeholder="Enter 11-digit GCash number">
                        </div>
                        <div class="form-group">
                            <label for="gcash_name">GCash Account Name</label>
                            <input type="text" id="gcash_name" name="gcash_name" placeholder="Enter GCash account name">
                        </div>
                    </div>
                    <div id="coopBankFields" class="payment-fields d-none">
                        ...
                        <div class="form-group">
                            <label for="bank_account_name">Bank Account Name</label>
                            <input type="text" id="bank_account_name" name="bank_account_name" placeholder="Enter bank account name">
                        </div>
                    </div>
                    <div id="coopCashFields" class="payment-fields d-none">
                        <div class="form-group">
                            <label for="reference_number">Receipt Number</label>
                            <input type="text" id="reference_number" name="reference_number" placeholder="Enter receipt number">
                        </div>
                    </div>
                    <button type="submit" class="submit-payment">
                        <i class="bi bi-bank2"></i> Submit Cooperative Fund Payment
                    </button>
                </form>
                <div id="coopFundsList" class="mt-4"></div>
                <script>
                function showCoopPaymentFields() {
                    const paymentMethod = document.getElementById('cooperative_payment_method').value;
                    const fields = ['coopGcashFields', 'coopBankFields', 'coopCashFields'];
                    fields.forEach(id => document.getElementById(id).classList.add('d-none'));
                    if (paymentMethod === 'gcash') document.getElementById('coopGcashFields').classList.remove('d-none');
                    if (paymentMethod === 'bank') document.getElementById('coopBankFields').classList.remove('d-none');
                    if (paymentMethod === 'cash') document.getElementById('coopCashFields').classList.remove('d-none');
                }
                function fetchCoopFundsList() {
                    ...
                    .then(data => {
                        if (data.success && data.funds.length > 0) {
                            let html = `<div class='table-responsive'><table class='table table-bordered table-striped'><thead><tr><th>Amount</th><th>Method</th><th>Date</th><th>Status</th></tr></thead><tbody>`;
                            data.funds.filter(f => f.member === '<?= htmlspecialchars($userFirstName . ' ' . $userLastName) ?>').forEach(f => {
                                html += `<tr>
                                    <td>₱${f.amount}</td>
                                    <td>${f.payment_method}</td>
                                    <td>${f.paid_at}</td>
                                    <td><span class='badge ${f.status === 'Confirmed' ? 'bg-success' : 'bg-warning text-dark'}'>${f.status}</span></td>
                                </tr>`;
                            });
                            html += '</tbody></table></div>';
                            document.getElementById('coopFundsList').innerHTML = html;
                        } else {
                            document.getElementById('coopFundsList').innerHTML = '<div class="alert alert-info">No cooperative fund payments found.</div>';
                        }
                    })
                    .catch(() => {
                        document.getElementById('coopFundsList').innerHTML = '<div class="alert alert-danger">Failed to load cooperative funds.</div>';
                    });
                }
                document.getElementById('cooperativePaymentForm').addEventListener('submit', function(e) {
                    e.preventDefault();
                    const submitBtn = this.querySelector('button[type="submit"]');
                    const originalBtnText = submitBtn.innerHTML;
                    submitBtn.disabled = true;
                    ...
                    .then(data => {
                        if (data.success) {
                            Swal.fire({
                                icon: 'success',
                                title: 'Payment Submitted!',
                                text: 'Your cooperative fund payment is being processed.',
                                confirmButtonColor: '#198754'
                            });
                            this.reset();
                            fetchCoopFundsList();
