...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_payment'])) {
    ...
    try {
        ...
        // Generate receipt number with more professional format
        $receiptNumber = 'TEBZ-' . date('Ymd') . '-' . str_pad($paymentId, 4, '0', STR_PAD_LEFT) . '-' . strtoupper(substr(md5(uniqid()), 0, 4));

        // First, update membership_payments table with confirmed_at
        ...
        // Now that membership_payments is updated, handle payment_details
        $checkPaymentDetailsTable = "SHOW TABLES LIKE 'payment_details'";
        $tableExists = $conn->query($checkPaymentDetailsTable)->num_rows > 0;
        
        if ($tableExists) {
            // Check if there's an existing record in payment_details
            $checkExistingDetails = "SELECT receipt_number FROM payment_details WHERE receipt_number = ?";
            $checkStmt = $conn->prepare($checkExistingDetails);
            $checkStmt->bind_param('s', $paymentInfo['receipt_number']);
            $checkStmt->execute();
            $existingResult = $checkStmt->get_result();
            
            if ($existingResult->num_rows > 0) {
                // Update existing record
                $updateDetailsStmt = $conn->prepare("UPDATE payment_details SET receipt_number = ? WHERE receipt_number = ?");
                $updateDetailsStmt->bind_param('ss', $receiptNumber, $paymentInfo['receipt_number']);
                if (!$updateDetailsStmt->execute()) {
                    throw new Exception('Failed to update payment details: ' . $updateDetailsStmt->error);
                }
            } else {
        ...
        // Store receipt in user_receipts table with additional details
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'send_receipt') {
    ...
    try {
        ...
        if (empty($payment['receipt_number'])) {
            $receiptNumber = 'JEEP-' . date('Ymd') . '-' . str_pad($paymentId, 4, '0', STR_PAD_LEFT);
            ...
            if (!$stmt->execute()) {
                throw new Exception('Failed to update receipt number: ' . $stmt->error);
            }
            
            $payment['receipt_number'] = $receiptNumber;
        }

        // Get payment method and date from the correct columns
        ...
        if (!$stmt->execute()) {
            throw new Exception('Failed to store receipt: ' . $stmt->error);
        }
...
<body class="bg-gradient-to-br from-blue-50 to-indigo-50 min-h-screen">
    ...
    <script>
        ...
        function viewReceipt(paymentId) {
            const receiptWindow = window.open(`treasurer_dashboard.php?action=view_receipt&payment_id=${paymentId}`, '_blank');
                ...
                receiptWindow.document.write(`
                    <div style="display: flex; justify-content: center; align-items: center; height: 100vh; background: #f3f4f6;">
                        <div style="text-align: center;">
                            ...
                            <p style="color: #4b5563;">Loading receipt...</p>
                        </div>
                    </div>
                `);
            }
        ...
        // Enhanced receipt sending function
        function sendReceipt(paymentId) {
            Swal.fire({
                title: 'Send Receipt',
                html: `
                    <div class="text-left">
                        <p class="mb-4">Are you sure you want to send the receipt to the member?</p>
                        <div class="bg-blue-50 p-4 rounded-lg">
                            <p class="text-sm text-blue-800">The receipt will be sent via:</p>
                            <ul class="list-disc list-inside text-sm text-blue-600 mt-2">
                                <li>Email (if available)</li>
                                <li>SMS (if phone number is available)</li>
                            </ul>
                        </div>
                    </div>
                `,
                    ...
                    Swal.fire({
                        ...
                        html: `
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-gray-600">Please wait while we send the receipt...</p>
                            </div>
                        `,
                        allowOutsideClick: false,
                        allowEscapeKey: false,
                    ...
                    .then(data => {
                        if (data.success) {
                            Swal.fire({
                                ...
                                html: `
                                    <div class="text-left">
                                        <p class="mb-4">The receipt has been sent successfully.</p>
                                        <div class="bg-green-50 p-4 rounded-lg">
                                            <p class="text-sm text-green-800">Delivery Status:</p>
                                            <p class="text-sm text-green-600 mt-2">✓ Email sent (if available)</p>
                                            <p class="text-sm text-green-600">✓ SMS sent (if available)</p>
                                        </div>
                                    </div>
                                `,
                                confirmButtonColor: '#4f46e5'
