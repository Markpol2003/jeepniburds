<?php
// Prevent any output before JSON response
error_reporting(E_ALL);
ini_set('display_errors', 0);
ob_start(); // Start output buffering

session_start();
require_once 'db_config.php';

// Set JSON header
header('Content-Type: application/json');

// Basic response function
function jsonResponse($success, $message, $data = null) {
    ob_clean(); // Clear any output buffer
    $response = [
        'success' => $success,
        'message' => $message
    ];
    if ($data !== null) {
        $response['receipt'] = $data; // Changed from 'data' to 'receipt' to match frontend
    }
    echo json_encode($response);
    exit;
}

// Check authentication
if (!isset($_SESSION['user_id'])) {
    jsonResponse(false, 'User not authenticated');
}

// Check request method
if ($_SERVER['REQUEST_METHOD'] !== 'POST' || !isset($_POST['payment_method'])) {
    jsonResponse(false, 'Invalid request');
}

try {
    $userId = $_SESSION['user_id'];
    $userFirstName = $_SESSION['user_firstName'] ?? '';
    $userLastName = $_SESSION['user_lastName'] ?? '';
    $userType = $_SESSION['user_type'] ?? '';
    
    $paymentMethod = $_POST['payment_method'];
    
    // Simple validation
    if (!in_array($paymentMethod, ['gcash', 'bank', 'cash'])) {
        jsonResponse(false, 'Invalid payment method');
    }
    
    // Validate required fields based on payment method
    $requiredFields = [
        'gcash' => ['gcash_number', 'gcash_name'],
        'bank' => ['bank_name', 'bank_account', 'bank_account_name'],
        'cash' => ['reference_number']
    ];

    // Check if all required fields are present
    if (!isset($requiredFields[$paymentMethod])) {
        throw new Exception('Invalid payment method');
    }

    foreach ($requiredFields[$paymentMethod] as $field) {
        if (empty($_POST[$field])) {
            throw new Exception("Missing required field: $field");
        }
    }

    // Generate receipt number
    $receiptNumber = 'TEBZ-' . date('Ymd') . '-' . str_pad($userId, 4, '0', STR_PAD_LEFT);
    $amount = 1000; // Fixed amount

    // Start transaction
    $conn->begin_transaction();

    try {
        // Get treasurer_id
        $treasurerQuery = "SELECT id FROM users WHERE userType = 'treasurer' LIMIT 1";
        $treasurerResult = $conn->query($treasurerQuery);
        if (!$treasurerResult || $treasurerResult->num_rows === 0) {
            throw new Exception('Treasurer account not found');
        }
        $treasurer = $treasurerResult->fetch_assoc();
        $treasurerId = $treasurer['id'];

        // Insert payment
        $stmt = $conn->prepare("INSERT INTO membership_payments (user_id, amount, method, receipt_number, status, payment_date) VALUES (?, ?, ?, ?, 'Pending', NOW())");
        if (!$stmt) {
            throw new Exception('Failed to prepare payment statement: ' . $conn->error);
        }
        
        $stmt->bind_param("idss", $userId, $amount, $paymentMethod, $receiptNumber);
        
        if (!$stmt->execute()) {
            throw new Exception('Failed to save payment record');
        }

        // Store payment details based on method
        $paymentDetails = [
            'receipt_number' => $receiptNumber,
            'amount' => $amount,
            'method' => $paymentMethod,
            'date' => date('Y-m-d H:i:s'),
            'name' => $userFirstName . ' ' . $userLastName,
            'status' => 'Pending'
        ];

        // Get treasurer payment details
        $treasurerDetailsQuery = "SELECT * FROM treasurer_payment_details WHERE treasurer_id = ? ORDER BY created_at DESC LIMIT 1";
        $treasurerDetailsStmt = $conn->prepare($treasurerDetailsQuery);
        $treasurerDetailsStmt->bind_param("i", $treasurerId);
        $treasurerDetailsStmt->execute();
        $treasurerDetails = $treasurerDetailsStmt->get_result()->fetch_assoc();

        if (!$treasurerDetails) {
            throw new Exception('Treasurer payment details not found');
        }

        // Store user's payment details
        switch ($paymentMethod) {
            case 'gcash':
                $stmt = $conn->prepare("INSERT INTO payment_details (receipt_number, gcash_number, gcash_name) VALUES (?, ?, ?)");
                $stmt->bind_param("sss", $receiptNumber, $_POST['gcash_number'], $_POST['gcash_name']);
                break;
            case 'bank':
                $stmt = $conn->prepare("INSERT INTO payment_details (receipt_number, bank_name, bank_account, bank_account_name) VALUES (?, ?, ?, ?)");
                $stmt->bind_param("ssss", $receiptNumber, $_POST['bank_name'], $_POST['bank_account'], $_POST['bank_account_name']);
                break;
            case 'cash':
                $stmt = $conn->prepare("INSERT INTO payment_details (receipt_number, reference_number) VALUES (?, ?)");
                $stmt->bind_param("ss", $receiptNumber, $_POST['reference_number']);
                break;
        }

        if (!$stmt->execute()) {
            throw new Exception('Failed to save payment details');
        }

        // Add treasurer details to payment details
        switch ($paymentMethod) {
            case 'gcash':
                $paymentDetails['gcash_number'] = $_POST['gcash_number'];
                $paymentDetails['gcash_name'] = $_POST['gcash_name'];
                $paymentDetails['treasurer_gcash'] = $treasurerDetails['gcash_number'];
                $paymentDetails['treasurer_gcash_name'] = $treasurerDetails['gcash_name'];
                break;
            case 'bank':
                $paymentDetails['bank_name'] = $_POST['bank_name'];
                $paymentDetails['bank_account'] = $_POST['bank_account'];
                $paymentDetails['bank_account_name'] = $_POST['bank_account_name'];
                $paymentDetails['treasurer_bank'] = $treasurerDetails['bank_name'];
                $paymentDetails['treasurer_account'] = $treasurerDetails['bank_account'];
                $paymentDetails['treasurer_account_name'] = $treasurerDetails['bank_account_name'];
                break;
            case 'cash':
                $paymentDetails['reference_number'] = $_POST['reference_number'];
                $paymentDetails['office_address'] = $treasurerDetails['office_address'];
                break;
        }

        // Commit transaction
        $conn->commit();

        jsonResponse(true, 'Payment submitted successfully', $paymentDetails);

    } catch (Exception $e) {
        // Rollback transaction on error
        $conn->rollback();
        throw $e;
    }

} catch (Exception $e) {
    jsonResponse(false, 'Error: ' . $e->getMessage());
}

