...
if ($action === 'boarding_history') {
    ...
    // We need to account for the fact that DATETIME values might be stored in UTC
    // or in the MySQL server's system timezone. To be safe, we'll query with a wider range
    // that accounts for potential timezone differences, or use DATE() which should work
    // with the session timezone if it's set correctly.
    
    // First, let's verify the MySQL session timezone is set correctly
    // Then use DATE() function which respects the session timezone
    // This is the most reliable approach since DATE() uses the session timezone context
    
    // Get boarding history with passenger details - ONLY for this driver
    // Use DATE() function which will use the session timezone (set to +08:00)
    // This should correctly extract the date part in Asia/Manila timezone
