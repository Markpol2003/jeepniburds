...
if ($action === 'boarding_history') {
    ...
    // This ensures we're comparing the correct day regardless of how data is stored
    $timezone = new DateTimeZone('Asia/Manila');
    $startDate = new DateTime($dateFilter . ' 00:00:00', $timezone);
    $endDate = new DateTime($dateFilter . ' 23:59:59', $timezone);
    $endDate->modify('+1 second'); // Include the last second of the day
    
    // Since MySQL session timezone is set to Asia/Manila (+08:00),
    // we can use these datetime values directly in the query
    // MySQL will interpret them in the session timezone context
    ...
    // Use date range comparison to avoid timezone issues with DATE() function
    // This works correctly as long as MySQL session timezone is set (which it is)
