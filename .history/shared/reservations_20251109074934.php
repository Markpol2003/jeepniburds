...
if ($action === 'driver_stats') {
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) AS boarded_today, AVG(TIMESTAMPDIFF(SECOND, here_at, boarded_at)) AS avg_wait
            FROM reservations 
            WHERE driver_id = ? AND DATE(boarded_at) = CURDATE() AND status = 'boarded'");
...
if ($action === 'boarding_history') {
    ...
    // Format for MySQL (the datetime values are already in the correct timezone context)
    // Since MySQL timezone is set, we can use these directly
    ...
    // Use date range instead of DATE() function to avoid timezone issues
