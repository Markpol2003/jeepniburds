...
if ($action === 'boarding_history') {
    ...
    // Since DATETIME doesn't store timezone, we need to interpret it
    // Most MySQL servers store DATETIME in system timezone (often UTC)
    // We'll try UTC first (most common), and if that doesn't work, try Manila timezone
    while ($result && $r = $result->fetch_assoc()) {
        if ($r['boarded_at']) {
            $boardedDate = null;
            
            // Try UTC interpretation first (most common case)
            try {
                $boardedDt = new DateTime($r['boarded_at'], new DateTimeZone('UTC'));
                $boardedDt->setTimezone($manilaTz);
                $boardedDateUtc = $boardedDt->format('Y-m-d');
                
                // Also try Manila timezone interpretation (if data was stored after timezone was set)
                $boardedDtLocal = new DateTime($r['boarded_at']);
                // Create a DateTime in Manila timezone by parsing the string as Manila time
                $boardedDtManila = DateTime::createFromFormat('Y-m-d H:i:s', $r['boarded_at'], $manilaTz);
                if ($boardedDtManila) {
                    $boardedDateManila = $boardedDtManila->format('Y-m-d');
                    // Use the interpretation that matches our target date, or UTC if both match
                    if ($boardedDateManila === $dateFilter) {
                        $boardedDate = $boardedDateManila;
                    } elseif ($boardedDateUtc === $dateFilter) {
                        $boardedDate = $boardedDateUtc;
                    }
                } else {
