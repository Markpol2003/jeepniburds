...
if ($action === 'driver_stats') {
    ...
    // Note: Since MySQL session timezone is set to +08:00 (Asia/Manila),
    // DATE_FORMAT already returns datetime in Manila time, so we parse it as Manila timezone
    while ($result && $r = $result->fetch_assoc()) {
        if ($r['boarded_at']) {
            try {
                // DATE_FORMAT returns datetime in session timezone (Manila), so parse as Manila time
                $boardedDt = DateTime::createFromFormat('Y-m-d H:i:s', $r['boarded_at'], $manilaTz);
                if ($boardedDt === false) {
                    // Fallback: try parsing as-is (might already be in correct timezone)
                    $boardedDt = new DateTime($r['boarded_at'], $manilaTz);
                }
...
if ($action === 'app_count') {
    ...
    $byTimePeriod = isset($_POST['by_time_period']) && $_POST['by_time_period'] === 'true';
    
    if ($byTimePeriod && $showAll) {
        // Count by time period across all dates
        // This groups all boarded passengers by their time of day (hour) regardless of date
        $stmt = $conn->prepare("SELECT 
        ...
        while ($row = $res->fetch_assoc()) {
            $hour = (int)$row['hour'];
            $periods[$hour] = (int)$row['c'];
        }
        $stmt->close();
        
        // Map to the specific time periods we care about (7-8, 8-9, 9-10, 10-11)
        $result = [
            '7-8' => 0,
            '8-9' => 0,
            '9-10' => 0,
        ...
        respond(true, ['periods' => $result]);
    } elseif ($showAll) {
        // Count all boarded reservations regardless of date/time
        $stmt = $conn->prepare("SELECT COUNT(*) AS c FROM reservations WHERE driver_id=? AND status='boarded' AND boarded_at IS NOT NULL");
        $stmt->bind_param('i', $driverId);
        $stmt->execute();
        $res = $stmt->get_result();
...
if ($action === 'boarding_history') {
    ...
    // If date is 'all' or empty, show all records
    $dateFilter = isset($_POST['date']) ? $_POST['date'] : date('Y-m-d');
    $showAll = ($dateFilter === 'all' || $dateFilter === '' || $dateFilter === null);
    
    $manilaTz = new DateTimeZone('Asia/Manila');
    
    if ($showAll) {
        // Get all boarding records without date filter - no limit to show all historical records
        $stmt = $conn->prepare("SELECT 
                    r.id,
                    r.passenger_id,
                    u.firstName,
        ...
        // Convert all dates to Manila timezone for display
        // Note: Since MySQL session timezone is set to +08:00 (Asia/Manila),
        // DATE_FORMAT already returns datetime in Manila time, so we parse it as Manila timezone
        while ($result && $r = $result->fetch_assoc()) {
            if ($r['boarded_at']) {
                try {
                    ...
                    $r['boarding_date'] = $boardedDt->format('Y-m-d');
                    // Also update boarded_at to ensure it's in Manila timezone for display
                    $r['boarded_at'] = $boardedDt->format('Y-m-d H:i:s');
                    if ($r['here_at']) {
                        $hereDt = DateTime::createFromFormat('Y-m-d H:i:s', $r['here_at'], $manilaTz);
                        if ($hereDt === false) {
                            $hereDt = new DateTime($r['here_at'], $manilaTz);
                        }
                        $r['here_at'] = $hereDt->format('Y-m-d H:i:s');
                    }
                    $rows[] = $r;
                } catch (Exception $e) {
                    // If parsing fails, use the original value (shouldn't happen, but safe fallback)
                    $r['boarding_date'] = substr($r['boarded_at'], 0, 10); // Extract date part YYYY-MM-DD
                    $rows[] = $r;
                }
            }
        }
    ...
    respond(true, ['history' => $rows, 'date' => $showAll ? 'all' : $dateFilter]);
}
