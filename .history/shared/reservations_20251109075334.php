...
if ($action === 'boarding_history') {
    ...
    // Try both UTC and local timezone interpretations
    while ($result && $r = $result->fetch_assoc()) {
        if ($r['boarded_at']) {
            $boardedDate = null;
            
            // Try interpreting as UTC (if MySQL system timezone is UTC)
            try {
                $boardedDtUtc = new DateTime($r['boarded_at'], new DateTimeZone('UTC'));
                $boardedDtUtc->setTimezone($manilaTz);
                $boardedDate = $boardedDtUtc->format('Y-m-d');
            } catch (Exception $e) {
                // If UTC interpretation fails, try as local time (if stored in session timezone)
                try {
                    $boardedDtLocal = new DateTime($r['boarded_at']);
                    // Assume it's already in Manila timezone if session timezone was set
                    $boardedDate = $boardedDtLocal->format('Y-m-d');
                } catch (Exception $e2) {
