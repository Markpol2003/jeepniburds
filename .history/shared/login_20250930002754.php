...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($password)) {
        header("Location: landing.php?login_error=Please enter both email and password");
        exit();
    }

    // Validate email format
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        header("Location: landing.php?login_error=Invalid email format");
        exit();
    }
    ...
    if ($user) {
        ...
        if (empty($user['userType'])) {
            ...
            header("Location: landing.php?login_error=User role is not defined. Please contact support.");
            exit();
        }
        ...
        if (password_verify($password, $user['password']) || $password === $user['password']) {
            ...
            header("Location: landing.php?login_error=Invalid password");
            exit();
        }
    } else {
        // User not found
        error_log("Login Error: User with email {$email} not found");
        header("Location: landing.php?login_error=Email not found");
        exit();
    }
}
...
function redirectToDashboard($role) {
    $dashboardRoutes = [
        ...
        'admin' => '../manager/manager_dashboard.php?page=cooperative_applications',
    ...
    if (array_key_exists($role, $dashboardRoutes)) {
        ...
        header("Location: landing.php?login_error=Invalid user role");
        exit();
    }
}
?>
