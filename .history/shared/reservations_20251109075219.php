...
if ($action === 'driver_stats') {
    ...
    // Check if MySQL timezone conversion is available
    $tzCheck = $conn->query("SELECT CONVERT_TZ('2024-01-01 00:00:00', 'UTC', '+08:00') AS test");
    $useConvertTz = $tzCheck !== false;
    ...
    if ($useConvertTz) {
        $stmt = $conn->prepare("SELECT COUNT(*) AS boarded_today, AVG(TIMESTAMPDIFF(SECOND, here_at, boarded_at)) AS avg_wait
                FROM reservations 
                WHERE driver_id = ? 
                AND DATE(CONVERT_TZ(boarded_at, @@session.time_zone, '+08:00')) = ?
                AND status = 'boarded'");
...
if ($action === 'boarding_history') {
    ...
    // The issue: DATE() function doesn't respect session timezone for DATETIME columns
    // DATETIME values are stored as-is without timezone info
    // If data was stored using NOW() when MySQL system timezone was UTC,
    // we need to account for the 8-hour difference (Asia/Manila is UTC+8)
    
    // Solution: Query using a date range that covers the entire day in Asia/Manila timezone
    // Convert the target date to UTC-equivalent range to catch all records
    // Asia/Manila is UTC+8, so Nov 9 00:00:00 Manila = Nov 8 16:00:00 UTC
    // and Nov 9 23:59:59 Manila = Nov 10 07:59:59 UTC
    
    // However, since we set MySQL session timezone to +08:00, NOW() returns correct time
    // But stored DATETIME values from before might be in UTC or system timezone
    // The safest approach: Query using DATE() but also check adjacent dates to be safe
    // OR: Use a date range that spans the potential timezone offset
    
    // Create date range: from start of day to end of day in Asia/Manila
    // Then adjust for potential UTC storage (subtract 8 hours for start, add 16 hours for end)
    $timezone = new DateTimeZone('Asia/Manila');
    $targetDate = new DateTime($dateFilter . ' 00:00:00', $timezone);
    
    // Convert to UTC to account for data that might be stored in UTC
    $targetDate->setTimezone(new DateTimeZone('UTC'));
    $startUtc = $targetDate->format('Y-m-d H:i:s');
    
    // End of day in Manila = start of next day minus 1 second, converted to UTC
    $endDate = new DateTime($dateFilter . ' 23:59:59', $timezone);
    $endDate->setTimezone(new DateTimeZone('UTC'));
    $endUtc = $endDate->format('Y-m-d H:i:s');
    
    // But wait - if MySQL session timezone is set correctly and data is stored using that timezone,
    // we should use the date range in the session timezone
    // Let's try a simpler approach: use DATE() with session timezone consideration
    // Since session timezone is +08:00, DATE() on a datetime stored at that timezone should work
    // But if old data was stored in UTC, we need to handle it differently
    
    // Most reliable: Query with DATE() using session timezone context
    // If that doesn't work due to old UTC data, fall back to date range query
    // Try the simple DATE() approach first
    $stmt = $conn->prepare("SELECT 
            ...
            AND (
                DATE(r.boarded_at) = ?
                OR DATE(r.boarded_at) = DATE(DATE_SUB(?, INTERVAL 1 DAY))
                OR DATE(r.boarded_at) = DATE(DATE_ADD(?, INTERVAL 1 DAY))
            )
            ORDER BY r.boarded_at DESC
            LIMIT 100");
    $stmt->bind_param('isss', $driverId, $dateFilter, $dateFilter, $dateFilter);
    $stmt->execute();
    $result = $stmt->get_result();
    $rows = [];
    $manilaTz = new DateTimeZone('Asia/Manila');
    while ($result && $r = $result->fetch_assoc()) {
        // Filter results to only include records that are actually on the target date in Manila timezone
        if ($r['boarded_at']) {
            // Parse the datetime and check if it's on the target date in Manila timezone
            $boardedDt = new DateTime($r['boarded_at'], new DateTimeZone('UTC')); // Assume stored in UTC
            $boardedDt->setTimezone($manilaTz);
            if ($boardedDt->format('Y-m-d') === $dateFilter) {
                $rows[] = $r;
            }
        } else {
            $rows[] = $r;
