...
if ($action === 'boarding_history') {
    ...
    // Create date range in Asia/Manila timezone (00:00:00 to 23:59:59.999999)
    $timezone = new DateTimeZone('Asia/Manila');
    $startDate = new DateTime($dateFilter . ' 00:00:00', $timezone);
    $endDate = new DateTime($dateFilter . ' 23:59:59', $timezone);
    
    // Format for MySQL query - these are the target dates in Asia/Manila
    ...
    // Use date range comparison with timezone conversion
    // CONVERT_TZ converts from UTC (if data was stored in UTC) to Asia/Manila (+08:00)
    // If MySQL timezone tables are not loaded, we'll use a simpler approach
    // First, try to detect if we need timezone conversion by checking stored times
    // Since MySQL session timezone is set, we'll use DATE() with the session timezone context
    // But to be absolutely sure, we'll convert the boarded_at to the session timezone first
    
    // Check if MySQL timezone tables are available, if not, use offset-based conversion
    $tzCheck = $conn->query("SELECT CONVERT_TZ('2024-01-01 00:00:00', 'UTC', '+08:00') AS test");
    $useConvertTz = $tzCheck !== false;
    
    if ($useConvertTz) {
        // Use CONVERT_TZ to ensure we're comparing dates in Asia/Manila timezone
        // This handles the case where data might have been stored in UTC
        $stmt = $conn->prepare("SELECT 
                    ...
                    DATE_FORMAT(CONVERT_TZ(r.boarded_at, @@session.time_zone, '+08:00'), '%Y-%m-%d') AS boarding_date
                FROM reservations r
                LEFT JOIN users u ON r.passenger_id = u.id
                WHERE r.driver_id = ? 
                AND r.status = 'boarded'
                AND DATE(CONVERT_TZ(r.boarded_at, @@session.time_zone, '+08:00')) = ?
                ORDER BY r.boarded_at DESC
                LIMIT 100");
    } else {
        // Fallback: Use date range comparison assuming session timezone is set correctly
        // Since session timezone should be +08:00, DATE() should work
        $stmt = $conn->prepare("SELECT 
                    r.id,
                    r.passenger_id,
                    u.firstName,
