...
if ($action === 'boarding_history') {
    ...
    // Convert Manila times to UTC for querying (assuming data stored in UTC)
    // Nov 9 00:00:00 Manila = Nov 8 16:00:00 UTC
    // Nov 9 23:59:59 Manila = Nov 9 15:59:59 UTC
    $startUtc = clone $startManila;
    $startUtc->setTimezone(new DateTimeZone('UTC'));
    
    $endUtc = clone $endManila;
    $endUtc->setTimezone(new DateTimeZone('UTC'));
    $endUtc->modify('+1 second'); // Make it exclusive upper bound
    
    $queryStart = $startUtc->format('Y-m-d H:i:s');
    $queryEnd = $endUtc->format('Y-m-d H:i:s');
