...
if ($action === 'boarding_history') {
    ...
    // Check MySQL system timezone to determine how data is stored
    $tzResult = $conn->query("SELECT @@global.time_zone AS global_tz, @@session.time_zone AS session_tz, NOW() AS current_time");
    $tzInfo = $tzResult ? $tzResult->fetch_assoc() : null;
    $mysqlSystemTz = $tzInfo['global_tz'] ?? 'SYSTEM';
    
    // Determine storage timezone: if global timezone is SYSTEM or UTC, assume UTC storage
    // Otherwise, assume data is stored in the session timezone (which we set to +08:00)
    $assumeUtcStorage = ($mysqlSystemTz === 'SYSTEM' || $mysqlSystemTz === '+00:00' || strpos($mysqlSystemTz, 'UTC') !== false);
    ...
    while ($result && $r = $result->fetch_assoc()) {
        if ($r['boarded_at']) {
            ...
            try {
                if ($assumeUtcStorage) {
                    // Data stored in UTC - convert to Manila timezone
                    $boardedDt = new DateTime($r['boarded_at'], new DateTimeZone('UTC'));
                    $boardedDt->setTimezone($manilaTz);
                } else {
                    // Data stored in session timezone (Asia/Manila) - use as-is
                    $boardedDt = new DateTime($r['boarded_at']);
                    // Verify it's actually in Manila timezone by checking offset
                    $offset = $boardedDt->getOffset();
                    if ($offset !== 28800) { // 28800 = 8 hours in seconds
                        // Not in Manila timezone, try UTC interpretation
                        $boardedDt = new DateTime($r['boarded_at'], new DateTimeZone('UTC'));
                        $boardedDt->setTimezone($manilaTz);
                    }
                }
            ...
            // Only include if the date matches (in Manila timezone)
            if ($boardedDate === $dateFilter) {
                // Format dates consistently - use the original values but ensure proper formatting
