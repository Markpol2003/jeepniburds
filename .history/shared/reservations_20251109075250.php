...
if ($action === 'boarding_history') {
    ...
    $endManila->modify('+1 second'); // Make it exclusive upper bound
    
    // Since DATETIME columns don't store timezone info, we need to determine
    // what timezone the data was stored in. The safest approach is to:
    // 1. Query a wider range that covers the day in both UTC and Manila time
    // 2. Filter results in PHP to only include records on the target date in Manila time
    
    // Query for records from 16:00 previous day to 16:00 next day (UTC)
    // This covers Nov 9 00:00-23:59 Manila time (which is Nov 8 16:00 to Nov 10 07:59 UTC)
    $prevDay = clone $startManila;
    $prevDay->modify('-1 day');
    $prevDay->setTime(16, 0, 0); // 16:00 previous day UTC = 00:00 target day Manila
    
    $nextDay = clone $endManila;
    $nextDay->setTime(16, 0, 0); // 16:00 target day UTC = 00:00 next day Manila
    
    // Convert to strings for MySQL query (assuming data might be in UTC)
    $queryStart = $prevDay->setTimezone(new DateTimeZone('UTC'))->format('Y-m-d H:i:s');
    $queryEnd = $nextDay->setTimezone(new DateTimeZone('UTC'))->format('Y-m-d H:i:s');
    
    // Get all boarding records in this range, then filter by actual Manila date
    ...
    while ($result && $r = $result->fetch_assoc()) {
        if ($r['boarded_at']) {
            // Try to parse as UTC first (most common if MySQL system timezone is UTC)
            try {
                $boardedDt = new DateTime($r['boarded_at'], new DateTimeZone('UTC'));
            } catch (Exception $e) {
                // If parsing fails, try without timezone
                $boardedDt = new DateTime($r['boarded_at']);
            }
            ...
            if ($boardedDate === $dateFilter) {
                ...
                $r['here_at'] = $r['here_at'] ? date('Y-m-d H:i:s', strtotime($r['here_at'])) : null;
                $r['boarded_at'] = $r['boarded_at'] ? date('Y-m-d H:i:s', strtotime($r['boarded_at'])) : null;
