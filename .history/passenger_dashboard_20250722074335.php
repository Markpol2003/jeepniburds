...
<div class="sidebar bg-gradient-dark text-white p-0" style="min-width: 280px; position: fixed; height: 100vh; overflow-y: auto;">
    ...
    <div class="sidebar-nav p-3">
        <ul class="nav flex-column gap-2">
            ...
            <!-- Jeepney Status Link -->
            <li class="nav-item">
                <a class="nav-link sidebar-link <?= $page === 'jeepney_status' ? 'active' : '' ?>" href="?page=jeepney_status">
                    <div class="d-flex align-items-center">
                        <div class="sidebar-icon">
                            <i class="bi bi-geo-alt-fill"></i>
                        </div>
                        <span class="sidebar-text">Jeepney Status</span>
                        <?php if ($page === 'jeepney_status'): ?>
                            <div class="sidebar-indicator"></div>
                        <?php endif; ?>
                    </div>
                </a>
...
.sidebar::-webkit-scrollbar-thumb:hover {
    ...
    <!-- Main Content -->
    <div class="content-wrapper" style="margin-left: 280px; width: calc(100% - 280px); height: 100vh; overflow: hidden;">
        <!-- Navbar -->
        <div class="d-flex align-items-center justify-content-center mb-3" style="padding: 15px 20px 0;">
            <img src="\tebz\img\logo12.png" alt="JeepniGo Logo" style="height: 35px; margin-right: 15px;">
            <h4 class="mb-0 fw-bold">Passenger Dashboard</h4>
        </div>

        <!-- Content Container -->
        <div class="content-container" style="height: calc(100vh - 80px); overflow-y: auto; padding: 0 20px 20px; display: flex; justify-content: center;">
            <div class="content <?= $page === 'dashboard' ? 'dashboard-page' : ($page === 'apply_driver_operator' ? 'announcement-page' : ($page === 'profile' ? 'profile-page' : ($page === 'apply_cooperative' ? 'cooperative-page' : ($page === 'jeepney_status' ? 'jeepney-status-page' : 'dashboard-page')))); ?>" style="max-width: 1200px; width: 100%;">
    <?php if ($page === 'dashboard'): ?>
        <!-- Dashboard -->
        ...
        .then(data => {
            if (data.success) {
                ...
                data.routes.forEach(route => {
                    ...
                    const cardHtml = `
                        <div class='status-card ${route.status.toLowerCase().replace(/\s+/g, '-')}' id='statusCard_${routeKey}'>
                            ...
                            <div class='boarding-events mt-2' id='boardingEvents_${routeKey}'>
                                <div class='text-muted small'>Loading recent boardings...</div>
                            </div>
                        </div>
                    `;
                    ...
                    // Fetch and display boarding events for this route
                    fetchBoardingEvents(route.route);
                });
...
function confirmPickup(route) {
    ...
    .then(data => {
        if (data.success) {
            ...
            fetchJeepneyStatus();
            fetchBoardingEvents(route); // Refresh events after confirming pickup
        } else {
...
function fetchBoardingEvents(route) {
    ...
    .then(data => {
        if (data.success && data.events.length > 0) {
            let html = `<div class="boarding-events-header d-flex align-items-center mb-2">
                <i class="bi bi-people-fill text-primary me-2" style="font-size: 1.3rem;"></i>
                <span class="fw-bold fs-6">Recent Boardings</span>
            </div>`;
            html += '<ul class="boarding-events-list list-group mb-0">';
            data.events.slice(0, 5).forEach((ev, idx) => {
                // Like logic: use sessionStorage to track likes per event (by event_time)
                const likeKey = `like_${routeKey}_${ev.event_time}`;
                const liked = sessionStorage.getItem(likeKey) === '1';
                html += `<li class="list-group-item d-flex align-items-center justify-content-between py-2 px-3 border-0 boarding-event-item">
                    <div class="d-flex align-items-center gap-2">
                        <span class='bi bi-person-circle text-primary fs-5'></span>
                        <span class='fw-semibold'>${ev.passenger}</span>
                        <span class='text-muted small ms-2'><i class="bi bi-clock"></i> ${new Date(ev.event_time).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                    </div>
                    <button class="btn btn-like p-0 border-0 bg-transparent" title="Like" onclick="likeBoardingEvent('${routeKey}','${ev.event_time}', this)" ${liked ? 'disabled' : ''}>
                        <i class="bi ${liked ? 'bi-hand-thumbs-up-fill text-success' : 'bi-hand-thumbs-up'} fs-5"></i>
                    </button>
                </li>`;
            });
            html += '</ul>';
...
// ... existing code ...
// Add likeBoardingEvent function
function likeBoardingEvent(routeKey, eventTime, btn) {
    const likeKey = `like_${routeKey}_${eventTime}`;
    sessionStorage.setItem(likeKey, '1');
    btn.querySelector('i').classList.remove('bi-hand-thumbs-up');
    btn.querySelector('i').classList.add('bi-hand-thumbs-up-fill', 'text-success');
    btn.disabled = true;
}
// ... existing code ...
</script>
...
<?php endif; ?>
    ...
    /* Jeepney Status Page - Keep original gradient but fit properly */
    .content.jeepney-status-page {
        background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
        min-height: 100%;
        padding: 1rem;
        border-radius: 15px;
