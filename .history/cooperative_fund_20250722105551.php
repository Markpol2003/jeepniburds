...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'contribute') {
        if (!isset($data['user_id'], $data['amount'], $data['payment_method'])) {
            echo json_encode(['success' => false, 'message' => 'Missing required fields.']);
            exit;
        }
        $stmt = $conn->prepare("INSERT INTO cooperative_fund_payments (user_id, amount, method) VALUES (?, ?, ?)");
        $stmt->bind_param('ids', $data['user_id'], $data['amount'], $data['payment_method']);
        if ($stmt->execute()) {
