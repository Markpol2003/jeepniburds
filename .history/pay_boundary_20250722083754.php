...
if (isset($data['driver_id'], $data['amount'], $data['payment_method'])) {
    try {
        ...
        if ($assignment) {
            ...
            // If no assignment, allow payment if operator_id is provided in POST data
            if (!empty($data['operator_id'])) {
                $operator_id = $data['operator_id'];
                $jeepney_id = 0; // or NULL if allowed by schema
            } else {
                throw new Exception('No active jeepney assignment found for this driver, and no operator_id provided. Please contact your operator.');
            }
        }
        ...
        if ($assignment) {
            ...
            $fixStmt->bind_param('ii', $data['driver_id'], $jeepney_id);
            $fixStmt->execute();
        }
        // Debug logging
        ...
        if (empty($data['driver_id']) || empty($operator_id) || empty($data['amount']) || empty($data['payment_method'])) {
            error_log("[Boundary Payment] Missing required fields: driver_id=" . ($data['driver_id'] ?? 'null') . ", operator_id=" . ($operator_id ?? 'null') . ", amount=" . ($data['amount'] ?? 'null') . ", payment_method=" . ($data['payment_method'] ?? 'null'));
        ...
        if (empty($operator_id) || $operator_id == 0) {
            ...
            throw new Exception('Invalid operator_id. Please contact your operator.');
        }
        // Validate amount
        if ($data['amount'] <= 0) {
