...
if ($action === 'list') {
    ...
    // Use CONVERT_TZ to ensure datetime is interpreted correctly
    // MySQL DATETIME doesn't store timezone, so we assume it's stored in server timezone or UTC
    // Convert to Manila timezone explicitly
    $stmt = $conn->prepare("SELECT fp.*, u.firstName, u.lastName,
                            CONVERT_TZ(fp.paid_at, @@session.time_zone, '+08:00') AS paid_at_manila
    ...
    while ($row = $result->fetch_assoc()) {
        // Convert paid_at to Manila timezone explicitly in PHP for safety
        $paidAtManila = $row['paid_at'];
        
        try {
            // Try to parse the datetime - assume it might be in UTC, server timezone, or Manila time
            // First, try parsing as-is (might already be in Manila time)
            $dt = new DateTime($row['paid_at']);
            
            // If we got a converted time from MySQL, use it
            if (!empty($row['paid_at_manila'])) {
                $paidAtManila = $row['paid_at_manila'];
            } else {
                // Otherwise, ensure it's in Manila timezone
                // Check if datetime was stored in UTC or server timezone
                // Since we set MySQL timezone to +08:00, MySQL should return it correctly
                // But to be safe, explicitly set timezone if not already set
        ...
        $fares[] = [
            ...
            'paid_at' => $paidAtManila, // Use Manila timezone converted value
    ...
    // Sort by paid_at timestamp (newest first)
    usort($fares, function($a, $b) use ($manilaTz) {
        try {
            $dtA = new DateTime($a['paid_at'], $manilaTz);
            $dtB = new DateTime($b['paid_at'], $manilaTz);
