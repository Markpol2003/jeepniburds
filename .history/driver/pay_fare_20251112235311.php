...
if ($action === 'count') {
    ...
    if ($byTimePeriod && $showAll) {
        ...
        // Parse route to extract origin and destination for fuzzy matching
        $routeParts = [];
        ...
        } else if (preg_match('/\s+(to|→)\s+/i', $route)) {
        ...
        // Get all trips - we'll filter by route in PHP
        ...
        while ($row = $tripsResult->fetch_assoc()) {
            ...
            // Method 1: Direct route field match
            if (!empty($row['route'])) {
                $tripRouteNormalized = $normalizeRoute($row['route']);
            ...
            if (!$routeMatches && (!empty($originPart) || !empty($destPart))) {
                $tripOrigin = !empty($row['origin_landmark']) ? strtolower(trim($row['origin_landmark'])) : '';
                $tripDest = !empty($row['dest_landmark']) ? strtolower(trim($row['dest_landmark'])) : '';
                
                $originMatch = empty($originPart);
                $destMatch = empty($destPart);
            ...
            // Method 3: Check if route field contains key parts
            if (!$routeMatches && !empty($row['route'])) {
                $tripRouteLower = strtolower($row['route']);
        ...
        // Get all collected fares - we'll filter by route in PHP
        ...
        while ($row = $faresResult->fetch_assoc()) {
            $hour = (int)$row['hour'];
            if (!isset($faresByHour[$hour])) {
                $faresByHour[$hour] = 0;
            }
            
            // Check route matching - use comprehensive matching similar to trips
            $routeMatches = false;
            if (!empty($row['route'])) {
                $fareRouteNormalized = $normalizeRoute($row['route']);
                $fareRouteLower = strtolower($row['route']);
                
                // Method 1: Direct normalized route match
                if ($fareRouteNormalized === $routeNormalized) {
                    $routeMatches = true;
                }
                
                // Method 2: Check if normalized routes are very similar (share most words)
                if (!$routeMatches) {
                    $routeWords = array_filter(explode(' ', $routeNormalized), function($w) { return strlen($w) > 2; });
                    $fareWords = array_filter(explode(' ', $fareRouteNormalized), function($w) { return strlen($w) > 2; });
                    if (count($routeWords) > 0 && count($fareWords) > 0) {
                        $commonWords = array_intersect($routeWords, $fareWords);
                        $similarity = count($commonWords) / max(count($routeWords), count($fareWords));
                        // If routes share at least 70% of words, consider them a match
                        if ($similarity >= 0.7) {
                            $routeMatches = true;
                        }
                    }
                ...
                // Method 3: Check if route contains key parts (fuzzy matching)
                if (!$routeMatches && !empty($originPart) && !empty($destPart)) {
                    // Check if route contains both origin and destination
                    if (strpos($fareRouteLower, $originPart) !== false && strpos($fareRouteLower, $destPart) !== false) {
                        $routeMatches = true;
                    } else if (!empty($originWords) && !empty($destWords)) {
                        // Check if route contains key words from both origin and destination
                        $originWordsFound = 0;
                        $destWordsFound = 0;
                        foreach ($originWords as $word) {
                            if (strpos($fareRouteLower, $word) !== false) {
                                $originWordsFound++;
                            }
                        }
                        foreach ($destWords as $word) {
                        ...
                        // Match if we found at least one word from origin and one from destination
                        if ($originWordsFound > 0 && $destWordsFound > 0) {
                            $routeMatches = true;
                        }
                    }
                } else if (!$routeMatches && !empty($originPart)) {
                    // Only origin specified
                    if (strpos($fareRouteLower, $originPart) !== false) {
                        $routeMatches = true;
                    } else if (!empty($originWords)) {
                        $matchCount = 0;
                        foreach ($originWords as $word) {
                ...
                } else if (!$routeMatches && !empty($destPart)) {
                    // Only destination specified
                    if (strpos($fareRouteLower, $destPart) !== false) {
                        $routeMatches = true;
                    } else if (!empty($destWords)) {
                        $matchCount = 0;
                        foreach ($destWords as $word) {
        ...
        // Set collected to equal total (assume 100% compliance)
        // Calculate rates
        foreach ($result as $key => &$data) {
            // Set collected to match total
            $data['compliant'] = $data['total'];
            // Rate is 100% if there are trips, 0% if no trips
            if ($data['total'] > 0) {
                $data['rate'] = 100.0;
            } else {
                $data['rate'] = 0.0;
