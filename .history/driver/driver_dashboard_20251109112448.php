                            ...
                            if(d && d.success){
                                ...
                                try {
                                    ...
                                    if (totalTrips > 0) {
                                        ...
                                        // If count is greater than totalTrips (due to data source differences), show 100%
                                        const percent = count >= totalTrips ? 100 : (count / totalTrips) * 100;
                ...
                function vmLoadAll(){
                    ...
                    Promise.all(promises).then(results => {
                        ...
                        const rows = results.map(r => {
                            ...
                            // If app boardings >= total trips, visibility is 100%
                            let visibilityPercent = null;
                            if (r.total > 0) {
                                // Calculate percentage and cap at 100%
                                // If count is greater than total (due to data source differences), show 100%
                                const percent = r.count >= r.total ? 100 : (r.count / r.total) * 100;
                                visibilityPercent = Math.min(percent, 100).toFixed(1);
                            } else if (r.count > 0) {
