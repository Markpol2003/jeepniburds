                    ...
                    <div class="card shadow-sm">
                        ...
                        <div class="card-body p-0">
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover table-striped mb-0" id="boardingHistoryTable">
                                    ...
                                    <tbody id="historyTableBody">
                                        <tr>
                                            <td colspan="9" class="text-center text-muted">No boarding records found for today</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
            ...
            (function(){
                ...
                window.loadBoardingHistory = function(){
                    ...
                    .then(r=>r.json()).then(d=>{
                        ...
                        if(!d.success || !d.history || d.history.length===0){
                            tbody.innerHTML = '<tr><td colspan="9" class="text-center text-muted">No boarding records found for this date</td></tr>';
                ...
                window.exportHistory = function(){
                    ...
                    .then(r=>r.json()).then(d=>{
                        ...
                        // Create CSV content
                        let csv = 'No,Passenger Name,Email,Route,From,To,Distance (km),Started Waiting,Boarded Time,Wait Time (min),Date\n';
                        d.history.forEach((h, idx) => {
                            const name = (h.firstName||'') + ' ' + (h.lastName||'');
                            const waitMin = Math.round((h.wait_seconds || 0) / 60);
                            csv += `${idx+1},"${name}","${h.email||''}","${h.route||''}","${h.origin_landmark||''}","${h.dest_landmark||''}",${h.distance_km||''},"${fmtTime(h.here_at)}","${fmtTime(h.boarded_at)}",${waitMin},"${fmtDate(h.boarding_date)}"\n`;
                        });
