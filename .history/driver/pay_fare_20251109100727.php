...
if ($action === 'list') {
    ...
    while ($row = $result->fetch_assoc()) {
        // Convert paid_at to Manila timezone explicitly
        // Strategy: Try parsing as UTC first (common default), then convert to Manila
        // If that fails, assume it's already in Manila time (since we set timezone on insert)
        $paidAtManila = $row['paid_at'];
        
        try {
            // First, try to parse as UTC (common if MySQL server uses UTC)
            try {
                $dt = new DateTime($row['paid_at'], $utcTz);
                $dt->setTimezone($manilaTz);
                $paidAtManila = $dt->format('Y-m-d H:i:s');
            } catch (Exception $e) {
                // If UTC parsing fails, try parsing as Manila time (naive datetime)
                // Create a datetime assuming it's in Manila timezone
