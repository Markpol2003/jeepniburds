    ...
    <div class="content-wrapper">
        ...
        <?php elseif ($page === 'collect_fares'): ?>
            ...
            (function(){
                ...
                // Auto-refresh compliance table every 30 seconds to keep it updated
                setInterval(window.fcLoadSlots, 30000);
            })();
            </script>
            <?php else: ?>
            ...
            function confirmCollected(receiptNumber, btn) {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        // Refresh the compliance table to reflect the new collected fare
                        if (typeof window.fcLoadSlots === 'function') {
                            window.fcLoadSlots();
                        }
                        
                        // Also refresh the fare list to update the status
                        fetchFares();
                        Swal.fire({
                            icon: 'success',
                    ...
                    if (showAll) {
                        ...
                        .then(r=>r.json()).then(d=>{
                            ...
                            if(d && d.success){
                                ...
                                tb.innerHTML = '<tr><td colspan="4" class="text-center text-muted">No boarding records found</td></tr>';
                                document.getElementById('vmTotal').textContent = 'Total Boarded: 0';
                            }
                        });
                        return;
