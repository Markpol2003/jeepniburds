...
if ($action === 'count') {
    ...
    $stmt = $conn->prepare("SELECT 
            COUNT(*) AS total,
            SUM(CASE WHEN status='Collected' THEN 1 ELSE 0 END) AS compliant
        FROM fare_payments WHERE route = ? AND paid_at BETWEEN ? AND ?");
    $stmt->bind_param('sss', $route, $startDt, $endDt);
    $stmt->execute();
    $res = $stmt->get_result();
    $row = $res ? $res->fetch_assoc() : ['total'=>0,'compliant'=>0];
    echo json_encode(['success'=>true,'total'=>(int)$row['total'],'compliant'=>(int)$row['compliant']]);
    exit;
}
