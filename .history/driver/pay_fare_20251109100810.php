...
if ($action === 'list') {
    ...
    while ($row = $result->fetch_assoc()) {
        // MySQL DATETIME columns don't store timezone info
        // Since we set MySQL session timezone to +08:00 (Asia/Manila),
        // the datetime value retrieved from MySQL represents Manila time
        // We need to explicitly create a DateTime object treating the value as Manila time
        $paidAtManila = $row['paid_at'];
        
        try {
            // Create DateTime object from the MySQL DATETIME string
            // Treat it as Manila timezone since that's what MySQL session timezone is set to
            // Use createFromFormat to explicitly set the timezone
            $dt = DateTime::createFromFormat('Y-m-d H:i:s', $row['paid_at'], $manilaTz);
            
            if ($dt !== false) {
                // Successfully parsed - format it to ensure it's in Manila timezone
                $paidAtManila = $dt->format('Y-m-d H:i:s');
            } else {
                // If createFromFormat fails, try parsing with DateTime constructor
                // But explicitly set timezone to Manila
                $dt = new DateTime($row['paid_at'], $manilaTz);
                $paidAtManila = $dt->format('Y-m-d H:i:s');
            }
        } catch (Exception $e) {
            // If parsing fails, use original value
            // This should rarely happen if MySQL returns valid datetime format
            $paidAtManila = $row['paid_at'];
        }
        ...
        $fares[] = [
            ...
            'paid_at' => $paidAtManila, // Explicitly in Manila timezone
