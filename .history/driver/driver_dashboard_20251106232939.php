...
<div class="d-flex">
    ...
    <div class="sidebar bg-gradient-dark text-white p-0" id="sidebar">
        ...
        <div class="sidebar-header p-4 text-center" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
            ...
            <!-- Manual vs App Counts & Visibility -->
            <div class="row g-3 mt-3">
                <div class="col-lg-6">
                    <div class="card shadow-sm">
                        <div class="card-header bg-secondary text-white d-flex justify-content-between align-items-center">
                            <div><i class="bi bi-clipboard-check"></i> Record Manual vs App Counts</div>
                            <div class="small">Visibility = (App / Manual) × 100</div>
                        </div>
                        <div class="card-body">
                            <div class="row g-2">
                                <div class="col-6">
                                    <label class="form-label small">Plate Number</label>
                                    <input type="text" id="vmPlate" class="form-control form-control-sm" value="<?= htmlspecialchars($assignedJeepney['plate_number'] ?? '') ?>" readonly>
                                </div>
                                <div class="col-6">
                                    <label class="form-label small">Route</label>
                                    <input type="text" id="vmRoute" class="form-control form-control-sm" value="<?= htmlspecialchars($assignedJeepney['route'] ?? '') ?>" readonly>
                                </div>
                                <div class="col-6">
                                    <label class="form-label small">Date</label>
                                    <input type="date" id="vmDate" class="form-control form-control-sm" value="<?= date('Y-m-d') ?>">
                                </div>
                                <div class="col-3">
                                    <label class="form-label small">Start</label>
                                    <input type="time" id="vmStart" class="form-control form-control-sm" value="07:00">
                                </div>
                                <div class="col-3">
                                    <label class="form-label small">End</label>
                                    <input type="time" id="vmEnd" class="form-control form-control-sm" value="08:00">
                                </div>
                            </div>
                            <hr class="my-2">
                            <div class="row g-2">
                                <div class="col-6">
                                    <label class="form-label small">Manual Ride In</label>
                                    <input type="number" id="vmRideIn" class="form-control form-control-sm" min="0" value="0">
                                </div>
                                <div class="col-6">
                                    <label class="form-label small">Manual Ride Out</label>
                                    <input type="number" id="vmRideOut" class="form-control form-control-sm" min="0" value="0">
                                </div>
                                <div class="col-6">
                                    <label class="form-label small">App Boarding (Auto)</label>
                                    <div class="input-group input-group-sm">
                                        <input type="number" id="vmAppIn" class="form-control" min="0" value="0">
                                        <button class="btn btn-outline-primary" type="button" id="vmComputeAppIn"><i class="bi bi-cpu"></i> Compute</button>
                                    </div>
                                </div>
                                <div class="col-6">
                                    <label class="form-label small">App Alighting (Optional)</label>
                                    <input type="number" id="vmAppOut" class="form-control form-control-sm" min="0" value="0">
                                </div>
                            </div>
                        </div>
                        <div class="card-footer bg-light d-flex gap-2">
                            <div class="btn-group">
                                <button class="btn btn-sm btn-outline-secondary" type="button" onclick="vmQuick('07:00','08:00')">7–8 AM</button>
                                <button class="btn btn-sm btn-outline-secondary" type="button" onclick="vmQuick('08:00','09:00')">8–9 AM</button>
                                <button class="btn btn-sm btn-outline-secondary" type="button" onclick="vmQuick('09:00','10:00')">9–10 AM</button>
                            </div>
                            <button class="btn btn-sm btn-primary ms-auto" type="button" id="vmSave"><i class="bi bi-save"></i> Save</button>
                        </div>
                    </div>
                </div>
                <div class="col-lg-6">
                    <div class="card shadow-sm">
                        <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center">
                            <div><i class="bi bi-table"></i> Visibility / Comparison</div>
                            <button class="btn btn-sm btn-light" type="button" id="vmRefresh"><i class="bi bi-arrow-repeat"></i></button>
                        </div>
                                ...
                                <table class="table table-striped table-hover mb-0">
                                    <thead class="table-light">
                                        <tr>
                                            ...
                                            <th>Start</th>
                                            <th>End</th>
                                            <th class="text-end">Ride In</th>
                                            <th class="text-end">App In</th>
                                            <th class="text-end">Vis In %</th>
                                            <th class="text-end">Ride Out</th>
                                            <th class="text-end">App Out</th>
                                            <th class="text-end">Vis Out %</th>
                                        </tr>
                                    </thead>
                                    <tbody id="vmRows">
                                        <tr>
            ...
            (function(){
                ...
                function buildRange(){
                    const d = document.getElementById('vmDate').value;
                    const s = document.getElementById('vmStart').value || '00:00';
                    const e = document.getElementById('vmEnd').value || '23:59';
                    return {
                        start: d + ' ' + s + ':00',
                        end: d + ' ' + e + ':00'
                    };
                ...
                function vmLoad(){
                    fetch('visibility_metrics.php', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({action:'list'})})
                    .then(r=>r.json()).then(d=>{
                        const tb = document.getElementById('vmRows');
                        if(!d.success || !d.rows || d.rows.length===0){ tb.innerHTML = '<tr><td colspan="10" class="text-center text-muted">No records yet</td></tr>'; return; }
                        tb.innerHTML = d.rows.map(r=>{
                            const vi = (r.visibility_in == null)? '-' : (r.visibility_in.toFixed(2)+'%');
                            const vo = (r.visibility_out == null)? '-' : (r.visibility_out.toFixed(2)+'%');
                            return `<tr>
                                <td>${r.plate_number||'-'}</td>
                                <td>${r.date||'-'}</td>
                                <td>${(r.start_time||'').slice(0,5)}</td>
                                <td>${(r.end_time||'').slice(0,5)}</td>
                                <td class="text-end">${r.ride_in||0}</td>
                                <td class="text-end">${r.app_in||0}</td>
                                <td class="text-end">${vi}</td>
                                <td class="text-end">${r.ride_out||0}</td>
                                <td class="text-end">${r.app_out||0}</td>
                                <td class="text-end">${vo}</td>
                            </tr>`;
                        }).join('');
                    });
                }
                function vmComputeAppIn(){
                    ...
                    .then(r=>r.json()).then(d=>{
                        if(d && d.success){ document.getElementById('vmAppIn').value = d.count||0; }
                        else { Swal.fire({icon:'info', title:'No Data', text: (d&&d.message)||'Unable to compute'}); }
                    });
                }
                function vmSave(){
                    const payload = {
                        action: 'save',
                        plate_number: document.getElementById('vmPlate').value,
                        date: document.getElementById('vmDate').value,
                        route: document.getElementById('vmRoute').value,
                        start_time: document.getElementById('vmStart').value,
                        end_time: document.getElementById('vmEnd').value,
                        ride_in: parseInt(document.getElementById('vmRideIn').value||'0'),
                        ride_out: parseInt(document.getElementById('vmRideOut').value||'0'),
                        app_in: parseInt(document.getElementById('vmAppIn').value||'0'),
                        app_out: parseInt(document.getElementById('vmAppOut').value||'0'),
                    };
                    fetch('visibility_metrics.php', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload)})
                    .then(r=>r.json()).then(d=>{
                        if(d && d.success){
                            Swal.fire({icon:'success', title:'Saved', timer:1200, showConfirmButton:false});
                            vmLoad();
                        } else {
                            Swal.fire({icon:'error', title:'Failed', text:(d&&d.message)||'Could not save'});
                        }
                    });
                }
                window.vmQuick = function(s,e){ document.getElementById('vmStart').value = s; document.getElementById('vmEnd').value = e; };
                const btnCompute = document.getElementById('vmComputeAppIn');
                const btnSave = document.getElementById('vmSave');
                const btnRefresh = document.getElementById('vmRefresh');
                if(btnCompute) btnCompute.addEventListener('click', vmComputeAppIn);
                if(btnSave) btnSave.addEventListener('click', vmSave);
                if(btnRefresh) btnRefresh.addEventListener('click', vmLoad);
