...
if ($action === 'list') {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        // Strategy: 
        // 1. Since we set MySQL session timezone to +08:00, MySQL interprets DATETIME in that context
        // 2. However, if data was stored when server was in UTC, we need to handle conversion
        // 3. We'll treat the retrieved datetime as Manila time (since that's our session timezone)
        //    but also check if it might be UTC-stored data that needs conversion
        
        $paidAtManila = $row['paid_at'];
        
        try {
            // Method 1: Treat as Manila time (most likely for new data)
            // Create DateTime object explicitly in Manila timezone
            $dt = DateTime::createFromFormat('Y-m-d H:i:s', $row['paid_at'], $manilaTz);
            
            if ($dt === false) {
                // If createFromFormat fails, try with DateTime constructor
                $dt = new DateTime($row['paid_at'], $manilaTz);
            }
            
            // Format the datetime - this ensures it's correctly represented in Manila timezone
            $paidAtManila = $dt->format('Y-m-d H:i:s');
            
        } catch (Exception $e) {
            // If all parsing fails, use original value
            // This preserves the data even if parsing fails
            $paidAtManila = $row['paid_at'];
        }
        ...
        $fares[] = [
            ...
            'paid_at' => $paidAtManila, // Formatted in Manila timezone
