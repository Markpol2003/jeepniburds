        ...
        <?php elseif ($page === 'collect_fares'): ?>
            ...
            <div class="card shadow-lg border-0 mb-4 animate__animated animate__fadeIn">
                <div class="card-header d-flex justify-content-between align-items-center" style="background: linear-gradient(90deg, #1abc9c 0%, #3498db 100%); border-top-left-radius: 1rem; border-top-right-radius: 1rem;">
                    <h5 class="mb-0 text-white d-flex align-items-center">
                            ...
                            <span id="faresHeaderDate" class="ms-2" style="font-size: 0.85rem; font-weight: normal;">(All Historical Records)</span>
                        ...
                        <div class="row align-items-center">
                            <div class="col-md-6">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="faresShowAll" checked>
                                    <label class="form-check-label" for="faresShowAll">
                                        <strong>Show All Historical Records</strong>
                                    </label>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <label for="faresDateFilter" class="form-label small mb-1">Filter by Date (optional):</label>
                                <input type="date" class="form-control form-control-sm" id="faresDateFilter" disabled style="max-width:200px">
                            </div>
                        </div>
                    </div>
                ...
                <div class="card-header bg-secondary text-white d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex gap-2 align-items-center">
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="checkbox" id="fcShowAll" 
                                   onchange="fcLoadSlots()">
                            <label class="form-check-label text-white" for="fcShowAll" style="font-size:0.875rem;">
                                Show All
                            </label>
                        </div>
                    </div>
            ...
            (function(){
                ...
                window.fcLoadSlots = function(){
                    const showAll = document.getElementById('fcShowAll')?.checked || false;
                    const date = document.getElementById('fcDate')?.value || '<?= date('Y-m-d') ?>';
                    const slots = [
                        {start:'07:00', end:'09:00', label:'7–9 AM', key:'7-9'},
                        {start:'08:00', end:'10:00', label:'8–10 AM', key:'8-10'},
                        {start:'17:00', end:'19:00', label:'5–7 PM', key:'5-7'}
                    ];
                    ...
                    if (document.getElementById('fcDate')) {
                        document.getElementById('fcDate').disabled = showAll;
                    }
                    
                    tb.innerHTML = '<tr><td colspan="6" class="text-center text-muted">Loading...</td></tr>';
                    
                    if (showAll) {
                        // Load all time periods with totals across all dates
                        fetch('pay_fare.php', {
                            method:'POST',
                        ...
                        .then(r=>r.json()).then(d=>{
                            if(d && d.success && d.periods){
                                const periods = d.periods;
                                const html = slots.map(slot => {
                                    const periodData = periods[slot.key] || {total: 0, compliant: 0, rate: 0};
                                    return `<tr>
                                        <td>${slot.label}</td><td>${plateNumber||'-'}</td><td>${route||'-'}</td>
                                        <td class='text-end'>${periodData.total}</td><td class='text-end'>${periodData.compliant}</td><td class='text-end'>${periodData.rate.toFixed(1)}%</td>
                                    </tr>`;
                                }).join('');
                ...
                if (fcDateEl) {
                    fcDateEl.addEventListener('change', function() {
                        if (!document.getElementById('fcShowAll')?.checked) {
                            window.fcLoadSlots();
                        }
                    });
                }
            ...
            function fetchFares() {
                ...
                // Check if we should filter by date or show all
                const showAll = document.getElementById('faresShowAll').checked;
                const dateFilter = document.getElementById('faresDateFilter').value;
                
                // Build request body
                const requestBody = { action: 'list' };
                ...
                .then(data => {
                    ...
                    if (data.success && data.fares.length > 0) {
                        ...
                        // Filter by date if needed (client-side filtering as backup)
                        let filteredFares = data.fares;
                        if (!showAll && dateFilter) {
                            filteredFares = data.fares.filter(fare => {
                                const fareDate = fare.paid_at ? fare.paid_at.split(' ')[0] : '';
                                return fareDate === dateFilter;
                            });
                        }
                        
                        // Ensure fares are sorted by paid_at (newest first) - additional client-side sort as safety
                        filteredFares.sort((a, b) => {
                        ...
                        const headerDateEl = document.getElementById('faresHeaderDate');
                        if (headerDateEl) {
                            if (showAll) {
                        ...
                        if (filteredFares.length === 0) {
                            html += `<tr><td colspan="9" class="text-center text-muted py-4">${showAll ? 'No fare payments found in history' : 'No fare payments found for this date'}</td></tr>`;
                        } else {
                        ...
                        // Update header even when no fares
                        const headerDateEl = document.getElementById('faresHeaderDate');
                        if (headerDateEl) {
                            const showAll = document.getElementById('faresShowAll').checked;
                            if (showAll) {
                ...
                .catch(() => {
                    ...
                    // Update header on error too
                    const headerDateEl = document.getElementById('faresHeaderDate');
                    if (headerDateEl) {
                        const showAll = document.getElementById('faresShowAll')?.checked ?? true;
                        if (showAll) {
                            headerDateEl.textContent = '(All Historical Records)';
                            headerDateEl.style.fontWeight = 'bold';
                        }
            ...
            const faresShowAllCheckbox = document.getElementById('faresShowAll');
            const faresDateFilter = document.getElementById('faresDateFilter');
            
            if (faresShowAllCheckbox) {
                faresShowAllCheckbox.addEventListener('change', function() {
                    // Enable/disable date filter based on checkbox
                    if (faresDateFilter) {
                        faresDateFilter.disabled = this.checked;
                        if (this.checked) {
                            faresDateFilter.value = '';
                        }
                    }
                    // Reload fares when filter changes
                    fetchFares();
                });
            }
            
            if (faresDateFilter) {
                faresDateFilter.addEventListener('change', function() {
                    // Only reload if "Show All" is unchecked
                    if (faresShowAllCheckbox && !faresShowAllCheckbox.checked) {
                        fetchFares();
                    }
                });
            }
                        ...
                        <div class="card-footer bg-light">
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <div class="btn-group btn-group-sm">
                                    ...
                                    <button class="btn btn-outline-primary" type="button" onclick="vmLoadPeriod('10:00','11:00')">10–11 AM</button>
            ...
            (function(){
                ...
                function fmtDate(dateStr){ 
                    ...
                    try {
                        ...
                        const match = dateStr.match(dateOnlyPattern);
                        ...
                        // It's a datetime string, use parseMySQLDateTime
                        const d = parseMySQLDateTime(dateStr);
                        if (!d) {
                ...
                function vmLoadPeriod(start, end){
                    ...
                    if (showAll) {
                        // If show all is checked, just call vmLoadAll to show all periods
                        vmLoadAll();
                        return;
                    }
                        ...
                        .then(r=>r.json()).then(async d=>{
                            ...
                            if(d && d.success){
                                ...
                                try {
                                    ...
                                    if (totalTrips > 0) {
                                        // Calculate percentage and cap at 100% to prevent values over 100%
                    ...
                    if (showAll) {
                        ...
                        .then(r=>r.json()).then(d=>{
                            ...
                            if(d && d.success && d.periods){
                                const periods = d.periods;
                                const periodLabels = [
                                    {key: '7-8', label: '7–8 AM'},
                                    {key: '8-9', label: '8–9 AM'},
                                    {key: '9-10', label: '9–10 AM'}
                                ];
                                
                                let total = 0;
                                const rows = periodLabels.map(p => {
                                    const count = periods[p.key] || 0;
                                    total += count;
                                    
                                    // Calculate visibility: 100% if there are boardings, 0% if none
                                    const visibilityPercent = count > 0 ? '100.0' : '0.0';
                                    const visibilityClass = parseFloat(visibilityPercent) >= 80 ? 'text-success' : 
                                                           parseFloat(visibilityPercent) >= 50 ? 'text-warning' : 'text-danger';
                                ...
                                document.getElementById('vmTotal').textContent = `Total Boarded (All Time): ${total}`;
                            } else {
                                tb.innerHTML = '<tr><td colspan="5" class="text-center text-muted">No boarding records found</td></tr>';
                                document.getElementById('vmTotal').textContent = 'Total Boarded: 0';
                            }
