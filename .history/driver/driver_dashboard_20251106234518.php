...
<div class="d-flex">
    ...
    <div class="content-wrapper">
        ...
        <?php elseif ($page === 'collect_fares'): ?>
            ...
            <!-- Fare Compliance Summary -->
            <div class="card shadow-sm mb-4">
                <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center">
                    <div><i class="bi bi-clipboard-data"></i> Fare Compliance Summary</div>
                    <button class="btn btn-sm btn-light" id="fcRefresh"><i class="bi bi-arrow-repeat"></i> Refresh</button>
                </div>
                <div class="card-body">
                    <div class="row g-2">
                        <div class="col-md-3">
                            <label class="form-label small">Plate</label>
                            <input class="form-control form-control-sm" id="fcPlate" value="<?= htmlspecialchars($assignedJeepney['plate_number'] ?? '') ?>" readonly>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label small">Route</label>
                            <input class="form-control form-control-sm" id="fcRoute" value="<?= htmlspecialchars($assignedJeepney['route'] ?? '') ?>" readonly>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label small">Date</label>
                            <input type="date" class="form-control form-control-sm" id="fcDate" value="<?= date('Y-m-d') ?>">
                        </div>
                        <div class="col-md-1">
                            <label class="form-label small">Start</label>
                            <input type="time" class="form-control form-control-sm" id="fcStart" value="07:00">
                        </div>
                        <div class="col-md-1">
                            <label class="form-label small">End</label>
                            <input type="time" class="form-control form-control-sm" id="fcEnd" value="09:00">
                        </div>
                        <div class="col-md-3">
                            <label class="form-label small">Manual Fare Checks (Sample Size)</label>
                            <input type="number" class="form-control form-control-sm" id="fcSample" min="0" value="0">
                        </div>
                        <div class="col-md-3">
                            <label class="form-label small">JeepniGo Paid Fares (Compliant Trips)</label>
                            <input type="number" class="form-control form-control-sm" id="fcCompliant" min="0" value="0" readonly>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label small">Total App-Logged Trips (Validation)</label>
                            <input type="number" class="form-control form-control-sm" id="fcTotal" min="0" value="0" readonly>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label small">Weight (Total ÷ Sample)</label>
                            <input type="text" class="form-control form-control-sm" id="fcWeight" value="-" readonly>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label small">Weighted Compliant Trips</label>
                            <input type="text" class="form-control form-control-sm" id="fcWeighted" value="-" readonly>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label small">Fare Compliance Rate (%)</label>
                            <input type="text" class="form-control form-control-sm" id="fcRate" value="-" readonly>
                        </div>
                    </div>
                </div>
                <div class="card-footer bg-light d-flex gap-2">
                    <div class="btn-group btn-group-sm">
                        <button class="btn btn-outline-secondary" type="button" onclick="fcQuick('07:00','09:00')">7–9 AM</button>
                        <button class="btn btn-outline-secondary" type="button" onclick="fcQuick('08:00','10:00')">8–10 AM</button>
                        <button class="btn btn-outline-secondary" type="button" onclick="fcQuick('17:00','19:00')">5–7 PM</button>
                    </div>
                    <button class="btn btn-sm btn-primary ms-auto" type="button" id="fcCompute"><i class="bi bi-cpu"></i> Compute</button>
                    <button class="btn btn-sm btn-success" type="button" id="fcSave"><i class="bi bi-save"></i> Save</button>
                </div>
            </div>

            <div class="card shadow-sm">
                <div class="card-header bg-secondary text-white"><i class="bi bi-table"></i> Compliance Records</div>
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table class="table table-striped table-hover mb-0">
                            <thead class="table-light">
                                <tr>
                                    <th>Plate</th><th>Route</th><th>Date</th><th>Start</th><th>End</th>
                                    <th class="text-end">Sample</th><th class="text-end">App Trips</th><th class="text-end">Compliant</th>
                                    <th class="text-end">Weight</th><th class="text-end">Weighted</th><th class="text-end">Rate %</th>
                                </tr>
                            </thead>
                            <tbody id="fcRows"><tr><td colspan="11" class="text-center text-muted">No records</td></tr></tbody>
                        </table>
                    </div>
                </div>
            </div>
            ...
            (function(){
                function fcBuildRange(){
                    ...
                    return { start: d+' '+s+':00', end: d+' '+e+':00' };
                }
                function fcCalc(sample, total, compliant){
                    const weight = (sample>0 && total>0)? (total/sample) : null;
                    const weighted = (weight!=null)? (compliant*weight) : null;
                    const rate = (total>0 && weighted!=null)? ((weighted/total)*100) : null;
                    return { weight, weighted, rate };
                }
                function fcRenderCalc(calc){
                    document.getElementById('fcWeight').value = (calc.weight==null? '-' : calc.weight.toFixed(2));
                    document.getElementById('fcWeighted').value = (calc.weighted==null? '-' : Math.round(calc.weighted));
                    document.getElementById('fcRate').value = (calc.rate==null? '-' : calc.rate.toFixed(1)+'%');
                }
                function fcLoadList(){
                    fetch('fare_compliance.php', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({action:'list'})})
                    .then(r=>r.json()).then(d=>{
                        const tb = document.getElementById('fcRows');
                        if(!d.success || !d.rows || d.rows.length===0){ tb.innerHTML = '<tr><td colspan="11" class="text-center text-muted">No records</td></tr>'; return; }
                        tb.innerHTML = d.rows.map(r=>{
                            return `<tr>
                                <td>${r.plate||'-'}</td><td>${r.route||'-'}</td><td>${r.date||'-'}</td>
                                <td>${(r.start||'').slice(11,16)}</td><td>${(r.end||'').slice(11,16)}</td>
                                <td class='text-end'>${r.sample||0}</td><td class='text-end'>${r.total||0}</td><td class='text-end'>${r.compliant||0}</td>
                                <td class='text-end'>${r.weight!=null? Number(r.weight).toFixed(2):'-'}</td><td class='text-end'>${r.weighted!=null? Math.round(r.weighted):'-'}</td>
                                <td class='text-end'>${r.rate!=null? Number(r.rate).toFixed(1)+'%':'-'}</td>
                            </tr>`;
                        }).join('');
                    });
                }
                function fcCompute(){
                    const range = fcBuildRange();
                    const route = document.getElementById('fcRoute').value;
                    fetch('pay_fare.php', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({action:'count', route: route, start_time: range.start, end_time: range.end})})
                    .then(r=>r.json()).then(d=>{
                        const total = d && d.success ? (d.total||0) : 0;
                        const compliant = d && d.success ? (d.compliant||0) : 0;
                        document.getElementById('fcTotal').value = total;
                        document.getElementById('fcCompliant').value = compliant;
                        const sample = parseInt(document.getElementById('fcSample').value||'0');
                        fcRenderCalc(fcCalc(sample, total, compliant));
                    });
                }
                function fcSave(){
                    const p = {
                        action:'save',
                        plate: document.getElementById('fcPlate').value,
                        route: document.getElementById('fcRoute').value,
                        date: document.getElementById('fcDate').value,
                        start: document.getElementById('fcDate').value+' '+document.getElementById('fcStart').value+':00',
                        end: document.getElementById('fcDate').value+' '+document.getElementById('fcEnd').value+':00',
                        sample: parseInt(document.getElementById('fcSample').value||'0'),
                        total: parseInt(document.getElementById('fcTotal').value||'0'),
                        compliant: parseInt(document.getElementById('fcCompliant').value||'0'),
                    };
                    fetch('fare_compliance.php', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(p)})
                    .then(r=>r.json()).then(d=>{
                        if(d && d.success){ Swal.fire({icon:'success', title:'Saved', timer:1000, showConfirmButton:false}); fcLoadList(); }
                        else { Swal.fire({icon:'error', title:'Failed', text:(d&&d.message)||'Could not save'}); }
                    });
                }
                window.fcQuick = function(s,e){ document.getElementById('fcStart').value=s; document.getElementById('fcEnd').value=e; };
                document.getElementById('fcCompute').addEventListener('click', fcCompute);
                document.getElementById('fcSave').addEventListener('click', fcSave);
                document.getElementById('fcRefresh').addEventListener('click', fcLoadList);
                document.addEventListener('DOMContentLoaded', fcLoadList);
            })();
            </script>
            <?php else: ?>
            ...
            function confirmCollected(receiptNumber, btn) {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        let statusCell = null;
                        if (tr) statusCell = tr.querySelector('td:nth-child(6) span');
                        if (statusCell) {
                ...
                <div class="col-12">
                    <div class="card shadow-sm">
                        <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                            <div><i class="bi bi-people-fill"></i> Boarded Passengers by Time Period</div>
                            <div class="d-flex gap-2 align-items-center">
            ...
            (function(){
                ...
                window.loadBoardingHistory = function(){
                    ...
                    .then(r=>r.json()).then(d=>{
                        ...
                        if(!d.success || !d.history || d.history.length===0){
                            tbody.innerHTML = '<tr><td colspan="8" class="text-center text-muted">No boarding records found for this date</td></tr>';
                ...
                window.exportHistory = function(){
                    ...
                    .then(r=>r.json()).then(d=>{
                        ...
                        a.href = url;
                        a.download = `boarding_history_${dateFilter}.csv`;
                        a.click();
                ...
                document.addEventListener('DOMContentLoaded', function(){ 
                    ...
                    if(dateFilterEl) dateFilterEl.addEventListener('change', loadBoardingHistory);
                });
                ...
                function vmLoadPeriod(start, end){
                    ...
                    .then(r=>r.json()).then(d=>{
                        ...
                        if(d && d.success){
                            ...
                            tb.innerHTML = '<tr><td colspan="4" class="text-center text-muted">No boarding records for this period</td></tr>';
                ...
                function vmLoadAll(){
                    ...
                    const promises = periods.map(p => {
                        ...
                        return fetch('../shared/reservations.php', {method:'POST', body: new URLSearchParams({action:'app_count', start_time: range.start, end_time: range.end})})
                            .then(r=>r.json())
                            .then(d=> ({period: p, count: (d && d.success) ? (d.count || 0) : 0}));
                    });
                    ...
                    Promise.all(promises).then(results => {
                        ...
                        if(rows === ''){
                            tb.innerHTML = '<tr><td colspan="4" class="text-center text-muted">No boarding records for today</td></tr>';
                        } else {
                            tb.innerHTML = rows;
                        }
                ...
                if(vmDateEl) {
                    vmDateEl.addEventListener('change', vmLoadAll);
