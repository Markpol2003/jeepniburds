<?php
// Prevent any output before JSON response
error_reporting(0);
ini_set('display_errors', 0);

session_start();
require_once 'db_config.php';

// Set JSON header
header('Content-Type: application/json');

// Debug session variables
error_log('Session variables: ' . print_r($_SESSION, true));

// Check if user is logged in and is an operator
if (!isset($_SESSION['user_id']) || !isset($_SESSION['user_type']) || strtolower($_SESSION['user_type']) !== 'operator') {
    echo json_encode([
        'success' => false,
        'message' => 'Unauthorized access'
    ]);
    exit();
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        // Get driver_id from POST data
        if (!isset($_POST['driver_id'])) {
            throw new Exception('Driver ID is required');
        }

        $driverId = $_POST['driver_id'];

        // First, verify if there's an active assignment
        $checkStmt = $conn->prepare("
            SELECT id FROM jeepney_assignments 
            WHERE driver_id = ? AND status = 'Active'
        ");
        
        if (!$checkStmt) {
            throw new Exception('Database error: ' . $conn->error);
        }

        $checkStmt->bind_param("i", $driverId);
        $checkStmt->execute();
        $result = $checkStmt->get_result();

        if ($result->num_rows === 0) {
            throw new Exception('No active assignment found for this driver');
        }

        // Update the jeepney assignment status to Inactive
        $stmt = $conn->prepare("
            UPDATE jeepney_assignments 
            SET status = 'Inactive',
                plate_number = NULL,
                body_number = NULL,
                route = NULL,
                notes = NULL
            WHERE driver_id = ? AND status = 'Active'
        ");

        if (!$stmt) {
            throw new Exception('Database error: ' . $conn->error);
        }

        $stmt->bind_param("i", $driverId);
        
        if (!$stmt->execute()) {
            throw new Exception('Failed to delete jeepney assignment: ' . $stmt->error);
        }

        if ($stmt->affected_rows === 0) {
            throw new Exception('No assignment was updated');
        }

        // Return success response
        echo json_encode([
            'success' => true,
            'message' => 'Jeepney assignment deleted successfully'
        ]);

    } catch (Exception $e) {
        echo json_encode([
            'success' => false,
            'message' => $e->getMessage()
        ]);
    }
} else {
    echo json_encode([
        'success' => false,
        'message' => 'Invalid request method'
    ]);
} 