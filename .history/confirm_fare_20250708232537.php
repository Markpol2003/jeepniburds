...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['fare_amount'])) {
    ...
    if ($route && $amount > 0) {
        $stmt = $conn->prepare('INSERT INTO fare_payments (passenger_id, amount, payment_method, route, paid_at, status) VALUES (?, ?, ?, ?, ?, ?)');
        $stmt->bind_param('idssss', $passengerId, $amount, $paymentMethod, $route, $paidAt, $status);
        if ($stmt->execute()) {
            // Success: redirect or show message
            $_SESSION['fare_success'] = 'Fare payment confirmed!';
            header('Location: passenger_dashboard.php?page=board_fare');
            exit();
        } else {
    ...
    <div class="row justify-content-center">
        <div class="col-md-6">
            <div class="card shadow-lg p-4">
                ...
                <?php endif; ?>
                <a href="passenger_dashboard.php?page=board_fare" class="btn btn-outline-primary mt-3">Back</a>
            </div>
        </div>
    </div>
</div>
