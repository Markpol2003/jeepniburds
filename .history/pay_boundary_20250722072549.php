...
if (isset($data['driver_id'], $data['amount'], $data['payment_method'])) {
    try {
        ...
        // Check for multiple active assignments (should not happen)
        $multiAssignStmt = $conn->prepare("SELECT COUNT(*) as cnt FROM jeepney_assignments WHERE driver_id = ? AND status = 'Active'");
        $multiAssignStmt->bind_param('i', $data['driver_id']);
        $multiAssignStmt->execute();
        $multiAssignResult = $multiAssignStmt->get_result();
        $multiAssignRow = $multiAssignResult->fetch_assoc();
        if ($multiAssignRow['cnt'] > 1) {
            // Optionally, set all but the most recent to Inactive
            $fixStmt = $conn->prepare("UPDATE jeepney_assignments SET status = 'Inactive' WHERE driver_id = ? AND status = 'Active' AND id != ?");
            ...
            try {
                ...
                if ($notificationResponse) {
                    ...
                    if ($notificationResult && $notificationResult['success']) {
                        error_log("Operator notification sent successfully for payment ID: $paymentId");
                    }
                }
            } catch (Exception $e) {
                error_log("Failed to send operator notification: " . $e->getMessage());
            }
