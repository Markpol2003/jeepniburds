...
function fetchBoardingEvents(route) {
    ...
    .then(data => {
        if (data.success && data.events.length > 0) {
            let html = `<div class='boarding-events-card p-3 mb-2'>
                <div class='boarding-events-title mb-2'><i class='bi bi-people-fill me-2'></i>Recent Boardings</div>
                <ul class='boarding-timeline list-unstyled mb-0'>`;
            data.events.slice(0, 5).forEach(ev => {
                html += `<li class='boarding-timeline-item d-flex align-items-center mb-2'>
                    <span class='timeline-dot me-2'></span>
                    <span class='boarding-passenger fw-semibold me-2'>${ev.passenger}</span>
                    <span class='boarding-time text-muted small'><i class='bi bi-clock me-1'></i>${new Date(ev.event_time).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                </li>`;
            });
            html += '</ul></div>';
            eventsDiv.innerHTML = html;
        } else {
            eventsDiv.innerHTML = `<div class='boarding-events-card p-3 mb-2'><div class='text-muted small'>No recent boardings.</div></div>`;
        }
    })
    .catch(() => {
        eventsDiv.innerHTML = `<div class='boarding-events-card p-3 mb-2'><div class='text-danger small'>Failed to load boardings.</div></div>`;
    });
}
</script>
<style>
