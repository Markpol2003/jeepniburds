...
if (file_exists($etaFile)) {
    ...
    foreach ($routes as &$r) {
        ...
        if (isset($etas[$key]['eta_minutes'])) {
            ...
            $r['eta_minutes'] = floatval($etas[$key]['eta_minutes']);
        }
        if (isset($etas[$key]['Wq'])) {
            $r['wq'] = floatval($etas[$key]['Wq']);
        }
        if (isset($etas[$key]['updated_at'])) {
            $r['eta_updated_at'] = $etas[$key]['updated_at'];
        }
    }
}
