...
if (isset($data['driver_id'], $data['amount'], $data['payment_method'])) {
    try {
        // Try to get the latest active assignment for the driver
        ...
        // Defensive: Always determine operator_id and jeepney_id
        if ($assignment) {
            $operator_id = $assignment['operator_id'];
            $jeepney_id = $assignment['jeepney_id'];
        } else if (!empty($data['operator_id']) && intval($data['operator_id']) > 0) {
            $operator_id = intval($data['operator_id']);
            $jeepney_id = 0;
        } else {
            error_log("[Boundary Payment] No active assignment and no valid operator_id provided for driver_id=" . ($data['driver_id'] ?? 'null'));
            echo json_encode(['success' => false, 'message' => 'No active jeepney assignment found for this driver, and no valid operator_id provided. Please contact your operator.']);
            exit;
        }

        // Defensive: operator_id must not be 0 or null
        if (empty($operator_id) || $operator_id == 0) {
            error_log("[Boundary Payment] Invalid operator_id: " . var_export($operator_id, true) . ". Payment not saved.");
            echo json_encode(['success' => false, 'message' => 'Invalid operator_id. Please contact your operator.']);
            exit;
        }

        error_log("[DEBUG] Inserting boundary payment: driver_id={$data['driver_id']}, operator_id={$operator_id}, jeepney_id={$jeepney_id}, amount={$data['amount']}");
        // Validate amount
        if ($data['amount'] <= 0) {
            throw new Exception('Amount must be greater than 0');
        }
        // Always generate a unique reference number for boundary payments
        $referenceNumber = 'BND-' . date('Ymd') . '-' . str_pad($data['driver_id'], 4, '0', STR_PAD_LEFT) . '-' . date('His');
...
if (isset($data['action']) && $data['action'] === 'list') {
    ...
    $operator_id = intval($data['operator_id']);
    error_log("[DEBUG] Fetching boundaries for operator_id=$operator_id");
